...
def get_targets(
    args: List[str], parser: optparse.OptionParser, use_null_terminator: bool
) -> List[str]:
...
def upload_to_google_storage(
    ...
    print(
        f"Warning: generation number could not be parsed from statusinfo: {err}"
    )
    return MISSING_GENERATION_MSG


def construct_deps_blob(
...
def main():
    ...
    parser.add_option(
        "-b", "--bucket", help="Google Storage bucket to upload to."
    )
    parser.add_option(
        "-p",
        "--prefix",
    ...
    if len(input_filenames) > 1 or (
        len(input_filenames) == 1 and os.path.isdir(input_filenames[0])
    ):
    ...
    if os.path.exists(GSUTIL_DEFAULT_PATH):
        ...
        for path in os.environ["PATH"].split(os.pathsep):
            if os.path.exists(path) and "gsutil" in os.listdir(path):
                gsutil = Gsutil(
                    os.path.join(path, "gsutil"), boto_path=options.boto
                )
        if not gsutil:
            parser.error(
    ...
    generation = upload_to_google_storage(
        file,
        base_url,
        object_name,
        gsutil,
    ...
    print(
        json.dumps(
            construct_deps_blob(options.bucket, object_name, file, generation),
            indent=2,
        )
    )
