...
class MetricsCollectorTest(unittest.TestCase):
    def setUp(self):
        ...
        mock.patch(
            "metrics.metrics_utils.print_notice", self.print_notice
        ).start()
        mock.patch(
            "metrics.metrics_utils.print_version_change",
            self.print_version_change,
        ).start()
        ...
        mock.patch(
            "metrics.metrics_utils.get_python_version", lambda: "2.7.13"
        ).start()
        mock.patch(
            "metrics.gclient_utils.GetOperatingSystem", lambda: "linux"
        ).start()
        mock.patch("metrics.detect_host_arch.HostArch", lambda: "x86").start()
        mock.patch(
            "metrics.metrics_utils.get_edit_monitor_state", lambda: None
        ).start()
    ...
    @mock.patch(
        "metrics.metrics_utils.get_edit_monitor_state", lambda: "enabled"
    )
    def test_collects_edit_monitor_state(self):
        ...
        self.assert_collects_metrics(
            {"env_vars": [{"name": "EDIT_MONITOR_STATE", "value": "enabled"}]}
        )
    ...
    def test_collects_is_cog_true(self):
        ...
        fun()
        self.assert_collects_metrics(
            {"env_vars": [{"name": "IS_COG", "value": "true"}]}
        )
...
class MetricsUtilsTest(unittest.TestCase):
    def test_extracts_host(self):
        ...
        self.assertEqual(
            "chromium-review.googlesource.com", http_metrics["host"]
        )
    ...
    def test_extracts_arguments(self):
        ...
        # Regular case
        http_metrics = metrics_utils.extract_http_metrics(
            "https://review.example.com/?q=123&foo=bar&o=ALL_REVISIONS",
            "",
            0,
            0,
        )
    ...
    def test_get_git_version(self, mockPopen):
        ...
        mockProcess.communicate.side_effect = [
            (b"git version 2.18.0.123.foo", "")
        ]
    ...
    def test_get_edit_monitor_state_linux(self, mock_call):
        ...
        with mock.patch.dict(os.environ, {"ENABLE_EDIT_MONITOR": ""}):
            self.assertEqual(
                metrics_utils.EditMonitorState.CONTROL,
        ...
        self.assertEqual(
            metrics_utils.EditMonitorState.ENABLED,
            metrics_utils.get_edit_monitor_state(),
        )
        ...
        with mock.patch.dict(os.environ, {"ENABLE_EDIT_MONITOR": "false"}):
            self.assertEqual(
                metrics_utils.EditMonitorState.DISABLED_OPT_OUT,
                metrics_utils.get_edit_monitor_state(),
            )
