...
class PresubmitUnittest(PresubmitTestsBase):
    ...
    def testListRelevantPresubmitFiles(self):
        ...
        os.listdir.side_effect = lambda d: (
            ["PRESUBMIT.py"] if d in dirs_with_presubmit else []
        )
    ...
    def testGitChange(self):
        ...
        scm.GIT.GenerateDiff.assert_called_once_with(
            self.fake_root_dir,
            files=[],
            full_move=True,
            branch="upstream",
    ...
    def testExecPresubmitScriptWithResultDB(self):
        ...
        sink.report.assert_called_with(
            "CheckChangeOnCommit", rdb_wrapper.STATUS_PASS, 0, None
        )
        ...
        sink.report.assert_called_with(
            "CheckChangeOnCommit", rdb_wrapper.STATUS_FAIL, 0, "error\n"
        )
    ...
    def testExecPostUploadHookSourceDirectory(self):
        ...
        executer = presubmit.GetPostUploadExecuter(
            change, presubmit.GerritAccessor()
        )
    ...
    def testDoPresubmitChecksNoWarningsOrErrors(self):
        ...
        os.path.isfile.side_effect = lambda f: (
            f in [root_path, haspresubmit_path]
        )
        os.listdir.return_value = ["PRESUBMIT.py"]
    ...
    def testDoPresubmitChecksJsonOutput(self):
        ...
        fake_error_locations = (
            '[output_api.PresubmitResultLocation(file_path="path/to/file")]'
        )
...
"""
    ...
    def testDoPresubmitChecksPromptsAfterWarnings(self):
        ...
        os.path.isfile.side_effect = lambda f: (
            f in [presubmit_path, haspresubmit_path]
        )
        os.listdir.return_value = ["PRESUBMIT.py"]
...
"""
        ...
        os.path.isfile.return_value = False
        os.listdir.side_effect = lambda d: (
            [] if d == self.fake_root_dir else ["PRESUBMIT.py"]
        )
        random.randint.return_value = 0

        change = self.ExampleChange(extra_lines=["STORY=http://tracker/123"])
        with (
            mock.patch("sys.stdin", StringIO("y\n")),
...
"""
    ...
    def testMainUnversioned(self, *_mocks):
        ...
        self.assertEqual(
            0, presubmit.main(["--root", self.fake_root_dir, "random_file.txt"])
        )
    ...
    def testMainUnversionedChecksFail(self, *_mocks):
        ...
        self.assertEqual(
            1, presubmit.main(["--root", self.fake_root_dir, "random_file.txt"])
        )
    ...
    def testParseChange_Files(self):
        ...
        scm.determine_scm.return_value = None
        options = mock.Mock(
            all_files=False, generate_diff=False, source_controlled_only=False
        )
    ...
    def testParseChange_DiffAndGenerateDiff(self):
        ...
        parser.error.side_effect = [SystemExit]
        options = mock.Mock(
            all_files=False, files=[], generate_diff=True, diff_file="foo.diff"
        )
    ...
    def testParseChange_NoFilesAndGit(self):
        ...
        scm.GIT.CaptureStatus.assert_called_once_with(
            options.root, options.upstream, ignore_submodules=False
        )
...
+bat"""
        ...
        options = mock.Mock(
            all_files=False, files=[], generate_diff=False, diff_file="foo.diff"
        )
        change = presubmit._parse_change(None, options)
    ...
    def testParseGerritOptions_GerritFetchNoUrl(self):
        ...
        parser.error.side_effect = [SystemExit]
        options = mock.Mock(
            gerrit_url=None, gerrit_fetch=True, issue=123, patchset=4
        )
...
class InputApiUnittest(PresubmitTestsBase):
    ...
    def testInputApiPresubmitScriptFiltering(self):
        ...
        self.assertEqual(
            got_files[0].LocalPath(), presubmit.normpath(files[0][1])
        )
        self.assertEqual(
            got_files[1].LocalPath(), presubmit.normpath(files[1][1])
        )
        self.assertEqual(
            got_files[2].LocalPath(), presubmit.normpath(files[2][1])
        )
        self.assertEqual(
            got_files[3].LocalPath(), presubmit.normpath(files[3][1])
        )
        self.assertEqual(
            got_files[4].LocalPath(), presubmit.normpath(files[4][1])
        )
        self.assertEqual(
            got_files[5].LocalPath(), presubmit.normpath(files[5][1])
        )
        self.assertEqual(
            got_files[6].LocalPath(), presubmit.normpath(files[6][1])
        )
        ...
        self.assertEqual(
            rhs_lines[0][0].LocalPath(), presubmit.normpath(files[0][1])
        )
        self.assertEqual(
            rhs_lines[3][0].LocalPath(), presubmit.normpath(files[0][1])
        )
        self.assertEqual(
            rhs_lines[7][0].LocalPath(), presubmit.normpath(files[4][1])
        )
        self.assertEqual(
            rhs_lines[13][0].LocalPath(), presubmit.normpath(files[4][1])
        )
    ...
    def testInputApiFilterSourceFile(self):
        files = [
            ...
            ["M", os.path.join("foo", "third_party", "WebKit", "WebKit.cpp")],
            ["M", os.path.join("foo", "third_party", "WebKit2", "WebKit2.cpp")],
            ["M", os.path.join("foo", "third_party", "blink", "blink.cc")],
            ["M", os.path.join("foo", "third_party", "blink1", "blink1.cc")],
        ...
        self.assertEqual(
            got_files[1].LocalPath(), presubmit.normpath(files[4][1])
        )
        self.assertEqual(
            got_files[2].LocalPath(), presubmit.normpath(files[5][1])
        )
        self.assertEqual(
            got_files[3].LocalPath(), presubmit.normpath(files[7][1])
        )
    ...
    def testDefaultFilesToCheckFilesToSkipFilters(self):
        ...
        for item in files:
            ...
            for i in range(len(results)):
                self.assertEqual(
                    results[i].LocalPath(), presubmit.normpath(item[1][i])
                )
            # Same number of expected results.
            self.assertEqual(
                sorted([f.UnixLocalPath() for f in results]), sorted(item[1])
            )
    ...
    def testGetAbsoluteLocalPath(self):
        ...
        self.assertEqual(
            affected_files[1].LocalPath(), normpath("isdir/blat.cc")
        )
    ...
    def testReadFileStringDenied(self):
        ...
        input_api = presubmit.InputApi(
            change, os.path.join(self.fake_root_dir, "/p"), False, None, False
        )
    ...
    def testReadFileAffectedFileDenied(self):
        fileobj = presubmit.AffectedFile(
            "boo", "M", "Unrelated", diff_cache=mock.Mock()
        )
    ...
    def testReadFileAffectedFileAccepted(self):
        fileobj = presubmit.AffectedFile(
            "AA/boo", "M", self.fake_root_dir, diff_cache=mock.Mock()
        )
        presubmit.gclient_utils.FileRead.return_code = None
    ...
    def testAppendCCAndMultipleChecks(self):
        ...
        self.assertEqual(
            [
                ...
                "ipc-security-reviews@chromium.org",
            ],
            executer.more_cc,
        )
...
class AffectedFileUnittest(PresubmitTestsBase):
    def testAffectedFile(self):
        ...
        af = presubmit.GitAffectedFile(
            "foo/blat.cc", "M", self.fake_root_dir, None
        )
    ...
    def testIsTestableFile(self):
        files = [
            presubmit.GitAffectedFile(
                "foo/blat.txt", "M", self.fake_root_dir, None
            ),
            presubmit.GitAffectedFile(
                "foo/binary.blob", "M", self.fake_root_dir, None
            ),
            presubmit.GitAffectedFile(
                "blat/flop.txt", "D", self.fake_root_dir, None
            ),
        ]
    ...
    def testGetUnixLocalPath(self):
        ...
        for path, expectedUnixLocalPath in cases:
            unixLocalPath = presubmit.GitAffectedFile(
                path, "M", self.fake_root_dir, None
            ).UnixLocalPath()
    ...
    def testGetExtension(self):
        cases = [
            ("foo/blat.txt", ".txt"),
            ("net/features.gni", ".gni"),
            ("archive.tar.gz", ".gz"),
            ("sub/archive.tar.gz", ".gz"),
            (".hidden", ""),
            ("sub/.hidden", ""),
            ("OWNERS", ""),
        ]
        ...
        for path, expectedExtension in cases:
            extension = presubmit.GitAffectedFile(
                path, "M", self.fake_root_dir, None
            ).Extension()
...
class ChangeUnittest(PresubmitTestsBase):
    def testAffectedFiles(self):
        change = presubmit.Change(
            "", "", self.fake_root_dir, [("Y", "AA"), ("A", "BB")], 3, 5, ""
        )
    ...
    def testAddDescriptionFooter_InvalidFooter(self):
        ...
        with self.assertRaises(ValueError):
            change.AddDescriptionFooter(
                "invalid.characters in:the", "footer key"
            )
...
class CannedChecksUnittest(PresubmitTestsBase):
    ...
    def DescriptionTest(
        self, check, description1, description2, error_type, committing
    ):
    ...
    def PythonLongLineTest(self, maxlen, content, should_pass):
        ...
        if should_pass:
            ...
            self.assertEqual(
                results[0].__class__, presubmit.OutputApi.PresubmitPromptWarning
            )
    ...
    def testCannedCheckChangeHasBugField(self):
        self.DescriptionTest(
            presubmit_canned_checks.CheckChangeHasBugField,
            "Foo\nBUG=b:1234",
            "Foo\n",
    ...
    def testCannedCheckChangeHasNoUnwantedTags(self):
        self.DescriptionTest(
            presubmit_canned_checks.CheckChangeHasNoUnwantedTags,
            "Foo\n",
            "Foo\nFIXED=1234",
    ...
    def testCheckChangeHasDescription(self):
        self.DescriptionTest(
            presubmit_canned_checks.CheckChangeHasDescription,
            "Bleh",
            "",
            presubmit.OutputApi.PresubmitNotifyResult,
            False,
    ...
    def testCannedCheckLargeScaleChange(self):
        ...
        result = results[0]
        self.assertEqual(
            result.__class__, presubmit.OutputApi.PresubmitPromptWarning
        )
    ...
    def testCannedCheckChangeHasNoTabs(self):
        ...
        self.assertEqual(
            results1[0].__class__, presubmit.OutputApi.PresubmitPromptWarning
        )
...
""",
    ...
    def testCannedCheckJSLongImports(self):
        ...
        self.ContentTest(
            ...
            "foo.js",
            "// We should import something long, eh?",
            "foo.js",
            presubmit.OutputApi.PresubmitPromptWarning,
        )
    ...
    def testCheckLicenseFailCommit(self):
        ...
        self._LicenseCheck(
            text, license_text, True, presubmit.OutputApi.PresubmitError
        )
    ...
    def testCheckLicenseEmptySuccess(self):
        ...
        self._LicenseCheck(
            text, license_text, True, None, accept_empty_files=True
        )
    ...
    def testCheckLicenseNewFileWarnWrongYearNoGerrit(self):
        ...
        result = presubmit_canned_checks.CheckLicense(
            input_api, presubmit.OutputApi, license_text, source_file_filter=42
        )
        self.assertEqual(len(result), 1)
        self.assertEqual(
            result[0].__class__, presubmit.OutputApi.PresubmitPromptWarning
        )
    ...
    def testCannedGetRuffFailureHint(self):
        ...
        results = cmd.output_parser(
            1, "file1.py:1:1: F401 'os' imported but unused"
        )
        self.assertEqual(len(results), 1)
        self.assertIn(
        ...
        # Exit code 0 returns None (no errors)
        self.assertIsNone(cmd.output_parser(0, ""))
        self.assertIsNone(cmd.output_parser(0, "   \n"))
        self.assertIsNone(cmd.output_parser(0, "All checks passed!\n"))
    ...
    def AssertOwnersWorks(
        ...
        response = response or {
            ...
            "labels": {
                "Code-Review": {
                    "all": [{"email": a, "value": +1} for a in approvers],
                    "default_value": 0,
                    "values": {
                        " 0": "No score",
                        "+1": "Looks good to me",
            ...
            "reviewers": {"REVIEWER": [{"email": a}] for a in approvers},
        }

        change = mock.MagicMock(presubmit.Change)
        change.OriginalOwnersFiles.return_value = {}
        change.RepositoryRoot.return_value = None
        change.ReviewersFromDescription.return_value = (
        ...
        with mock.patch(
            "owners_client.OwnersClient.ListOwners",
    ...
    def testCannedCheckOwners_TBRIgnored(self):
        ...
        self.AssertOwnersWorks(
            tbr=True, is_committing=False, expected_output=""
        )
    ...
    def testCannedRunUnitTests(self):
        ...
        results = presubmit_canned_checks.RunUnitTests(
            input_api, presubmit.OutputApi, unit_tests
        )
    ...
    def testCannedRunUnitTestsWithTimer(self):
        ...
        results = presubmit_canned_checks.RunUnitTests(
            input_api, presubmit.OutputApi, ["bar.py"]
        )
        self.assertEqual(
    ...
    def testCannedRunUnitTestsPython3(self):
        ...
        self.assertEqual(
            [result.__class__ for result in results],
            [
                presubmit.OutputApi.PresubmitPromptWarning,
                presubmit.OutputApi.PresubmitNotifyResult,
            ],
    ...
    def testCannedRunUnitTestsDontRunOnPython2(self):
        ...
        unit_tests = ["allo", "bar.py"]
        results = presubmit_canned_checks.RunUnitTests(
            input_api, presubmit.OutputApi, unit_tests, run_on_python2=False
        )
        self.assertEqual(
            [result.__class__ for result in results],
            [
    ...
    def testCheckCIPDPackages(self):
        ...
        self.assertEqual(
            command.cmd, ["cipd", "ensure-file-verify", "-ensure-file=-"]
        )
