...
class ReclientHelperTest(trial_dir.TestCase):
    ...
    @unittest.mock.patch.dict(
        os.environ, {"AUTONINJA_BUILD_ID": "SOME_RANDOM_ID"}, clear=True
    )
    @unittest.mock.patch(
        "reclient_helper.datetime_now",
        return_value=datetime.datetime(2017, 3, 16, 20, 0, 41, 0),
    )
    ...
    def test_ninja_reclient_sets_path_env_vars(self, *_):
        ...
        reclient_cfg = os.path.join(
            "src", "buildtools", "reclient_cfgs", "reproxy.cfg"
        )
        write(".gclient", "")
        ...
        self.assertTrue(
            os.path.isdir(
                os.path.join(
                    ...
                    hashlib.md5(
                        os.path.join(
                            self.root_dir, "out", "a", ".reproxy_tmp"
                        ).encode()
                    ).hexdigest(),
                )
            )
    ...
    @unittest.mock.patch("subprocess.call", return_value=0)
    @unittest.mock.patch("ninja.main", return_value=0)
    def test_ninja_reclient_calls_reclient_binaries(
        self, mock_ninja, mock_call
    ):
        reclient_bin_dir = os.path.join("src", "buildtools", "reclient")
        reclient_cfg = os.path.join(
            "src", "buildtools", "reclient_cfgs", "reproxy.cfg"
    ...
    @unittest.mock.patch.dict(
        os.environ, {"AUTONINJA_BUILD_ID": "SOME_RANDOM_ID"}
    )
    @unittest.mock.patch(
        "reclient_helper.get_hostname", return_value="somehost"
    )
    @unittest.mock.patch("subprocess.call", return_value=0)
    @unittest.mock.patch("ninja.main", return_value=0)
    def test_ninja_reclient_collect_metrics_cache_missing(self, *_):
        ...
        self.assertEqual(
            os.environ.get("RBE_metrics_project"), "chromium-reclient-metrics"
        )
        self.assertEqual(
            os.environ.get("RBE_metrics_table"), "rbe_metrics.builds"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            os.environ.get("RBE_metrics_prefix"), "go.chromium.org"
        )
    ...
    def test_ninja_reclient_collect_metrics_cache_valid(self, *_):
        ...
        cache_dir = os.path.join(
            ...
            ".reproxy_cache",
            hashlib.md5(
                os.path.join(self.root_dir, "out", "a", ".reproxy_tmp").encode()
            ).hexdigest(),
        )
        write(".gclient", "")
        write(".gclient_entries", 'entries = {"buildtools": "..."}')
...
expiry:  {
  seconds:  %d
    ...
    def test_ninja_reclient_cfg_not_found(self, mock_ninja, mock_call):
        ...
        write(".gclient_entries", 'entries = {"buildtools": "..."}')
        write(
            os.path.join("src", "buildtools", "reclient", "version.txt"), "0.0"
        )
        argv = ["ninja", "-C", "out/a", "chrome"]
