...
class GClientEvalTest(unittest.TestCase):
    ...
    def test_invalid_call(self):
        ...
        self.assertIn(
            "Str and Var are the only allowed functions", str(cm.exception)
        )
    ...
    def test_expands_vars_with_braces(self):
        ...
        self.assertEqual(
            "baz",
            gclient_eval._gclient_eval(
                '"{bar}"', vars_dict={"bar": gclient_eval.ConstantString("baz")}
            ),
        )
    ...
    def test_invalid_var(self):
        ...
        self.assertIn(
            "bar was used as a variable, but was not declared",
            str(cm.exception),
        )
    ...
    def test_format_attribute_traversal(self):
        with self.assertRaises(ValueError) as cm:
            gclient_eval._gclient_eval(
                '"{bar.__class__}"', vars_dict={"bar": "foo"}
            )
        self.assertIn(
...
class ExecTest(unittest.TestCase):
    ...
    def test_doesnt_override_undeclared_vars(self):
        ...
        self.assertIn(
            "baz was used as a variable, but was not declared",
            str(cm.exception),
        )
...
class ParseLocalConfigTest(unittest.TestCase):
    ...
    def test_unsafe_call(self):
        ...
        with self.assertRaises(gclient_utils.Error) as cm:
            gclient_eval.ParseLocalConfig(content, ".gclient")
        self.assertIn(
            "Error evaluating local config solutions", str(cm.exception)
        )
...
class EvaluateConditionTest(unittest.TestCase):
    ...
    def test_variable(self):
        self.assertFalse(
            gclient_eval.EvaluateCondition("foo", {"foo": "False"})
        )
    ...
    def test_str_in_condition(self):
        ...
        self.assertTrue(
            gclient_eval.EvaluateCondition(
                's_var == "foo"', {"s_var": Str("foo")}
            )
        )

        self.assertFalse(
            gclient_eval.EvaluateCondition(
                's_var in ("baz", "quux")', {"s_var": Str("foo")}
            )
        )
...
class RevisionTest(unittest.TestCase):
    def assert_gets_and_sets_revision(
        self, before, after, rev_before="deadbeef"
    ):
        local_scope = gclient_eval.Exec(file_join(before))
        ...
        self.assertEqual(
            file_join(after), gclient_eval.RenderDEPSFile(local_scope)
        )
...
class ParseTest(unittest.TestCase):
    ...
    def test_has_builtin_vars(self):
        ...
        local_scope = gclient_eval.Parse(
            deps_file, "<unknown>", None, builtin_vars
        )
        self.assertEqual(
            {
                "deps": {"a_dep": {"url": "afoob", "dep_type": "git"}},
            },
            local_scope,
        )
    ...
    def test_overrides_vars(self):
        ...
        self.assertEqual(
            {
                "vars": {"foo": "bar"},
                "deps": {"a_dep": {"url": "abazb", "dep_type": "git"}},
            },
            local_scope,
        )
    ...
    def test_merges_deps_os_extra_dep(self):
        ...
        self.assertEqual(
            {
                "deps": {
                    "a_dep": {"url": "a_url@a_rev", "dep_type": "git"},
                    "b_dep": {
                        "url": "b_url@b_rev",
                        "dep_type": "git",
