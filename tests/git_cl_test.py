...
class TestGitClBasic(unittest.TestCase):
    ...
    def test_main_untracked_cache_bug_find_git_dir(
        self, mock_find, mock_capture, mock_execute
    ):
        mock_execute.side_effect = subprocess2.CalledProcessError(
            128,
            ["git", "status"],
        ...
        lock_path = os.path.join(
            os.path.abspath("/path/to/found/.git"), "index.lock"
        )
        index_path = os.path.join(
            os.path.abspath("/path/to/found/.git"), "index"
        )
        expected_msg = (
            f"Git encountered a bug with the untracked cache.\n"
            f"To fix this, please rebuild your git index.\n"
    ...
    @mock.patch("scm.GIT.Capture")
    @mock.patch("git_cl.FindGitDir")
    def test_main_untracked_cache_bug_fallback(
        self, mock_find, mock_capture, mock_execute
    ):
        mock_execute.side_effect = subprocess2.CalledProcessError(
            128,
    ...
    def test_upload_to_non_default_branch_no_retry(self):
        m = mock.patch(
            "git_cl.Changelist._CMDUploadChange",
            side_effect=[git_cl.GitPushError(), None],
        ).start()
        mock.patch(
            "git_cl.Changelist.GetRemoteBranch", return_value=("foo", "bar")
        ).start()
        mock.patch(
            "git_cl.Changelist.GetGerritProject", return_value="foo"
        ).start()
        mock.patch(
            "git_cl.gerrit_util.GetProjectHead", return_value="refs/heads/main"
        ).start()
        ...
        with self.assertRaises(SystemExitMock):
            cl.CMDUploadChange(
                options, [], "foo", git_cl.ChangeDescription("bar")
            )
    ...
    def test_upload_to_old_default_still_active(self):
        ...
        mock.patch(
            ...
            return_value=("foo", git_cl.DEFAULT_OLD_BRANCH),
        ).start()
        mock.patch(
            "git_cl.Changelist.GetGerritProject", return_value="foo"
        ).start()
    ...
    def test_upload_with_message_file_no_editor(self):
        m = mock.patch(
            "git_cl.ChangeDescription.prompt", return_value=None
        ).start()
        mock.patch(
            "git_cl.Changelist.GetRemoteBranch",
            return_value=("foo", git_cl.DEFAULT_NEW_BRANCH),
        ).start()
        mock.patch(
            "git_cl.GetTargetRef", return_value="refs/heads/main"
        ).start()
        mock.patch(
        ...
        mock.patch(
            "git_cl.Changelist.GetBranch", side_effect=SystemExitMock
        ).start()
    ...
    def test_get_bug_line_values(self):
        ...
        self.assertEqual(
            f("v8:", "chromium:123,456"), ["v8:456", "chromium:123"]
        )
...
class TestParseIssueURL(unittest.TestCase):
    ...
    def test_gerrit_url(self):
        ...
        self._test(
            "http://chrome-review.source.com/c/123",
            123,
            None,
            "chrome-review.source.com",
        )
        self._test(
            "https://chrome-review.source.com/c/123/",
            123,
            None,
            "chrome-review.source.com",
        )
        self._test(
            "https://chrome-review.source.com/c/123/4",
            123,
            4,
            "chrome-review.source.com",
        )
        ...
        self._test(
            "https://chrome-review.source.com/123",
            123,
            None,
            "chrome-review.source.com",
        )
        ...
        self._test(
            "https://chrome-review.source.com/c/123/1/whatisthis", fail=True
        )
        self._test("https://chrome-review.source.com/c/abc/", fail=True)

    def test_short_urls(self):
        self._test(
            "https://crrev.com/c/2151934",
            2151934,
            None,
    ...
    def test_missing_scheme(self):
        self._test(
            "codereview.source.com/123", 123, None, "codereview.source.com"
        )
        self._test(
            "crrev.com/c/2151934",
            2151934,
    ...
    def test_corp_urls(self):
        self._test(
            "https://chromium-review.git.corp.google.com/c/123/4",
            123,
            4,
            "chromium-review.googlesource.com",
        )
        self._test(
            "chromium-review.git.corp.google.com/123",
            123,
            None,
            "chromium-review.googlesource.com",
        )
...
class GitCookiesCheckerTest(unittest.TestCase):
    ...
    def mock_hosts_creds(self, subhost_identity_pairs):
        ...
        self.c._all_hosts = [
            (ensure_googlesource(h), i, ".gitcookies")
            for h, i in subhost_identity_pairs
        ]

    def test_identity_parsing(self):
        self.assertEqual(
            self.c._parse_identity("ldap.google.com"), ("ldap", "google.com")
        )
        self.assertEqual(
            self.c._parse_identity("git-ldap.example.com"),
            ("ldap", "example.com"),
        )
        # Specical case because we know there are no subdomains in chromium.org.
        self.assertEqual(
            self.c._parse_identity("git-note.period.chromium.org"),
            ("note.period", "chromium.org"),
        )
        # Pathological: ".period." can be either username OR domain, more likely
        # domain.
        self.assertEqual(
            self.c._parse_identity("git-note.period.example.com"),
    ...
    def test_analysis(self):
        ...
        self.assertEqual(
            set(["conflict.googlesource.com"]), self.c.get_conflicting_hosts()
        )
        self.assertEqual(
            set(["dup.googlesource.com"]), self.c.get_duplicated_hosts()
        )
        self.assertEqual(
            set(
    ...
    def test_report(self, *_mocks):
        ...
        with open(
            os.path.join(
                os.path.dirname(__file__), "git_cl_creds_check_report.txt"
            )
        ) as f:
            expected = f.read() % {
                "sep": os.sep,
...
class TestGitCl(unittest.TestCase):
    def setUp(self):
        ...
        mock.patch("sys.stdout", io.StringIO()).start()
        mock.patch(
            "git_cl.time_time", lambda: self._mocked_call("time.time")
        ).start()
        mock.patch(
        ...
        mock.patch(
            ...
            lambda *a, **_k: ([self._mocked_call(*a), ""], 0),
        ).start()
        mock.patch(
        ...
        mock.patch(
            "git_cl.write_json", lambda *a: self._mocked_call("write_json", *a)
        ).start()
        mock.patch(
            "git_cl.Changelist.RunHook",
            return_value={"more_cc": ["test-more-cc@chromium.org"]},
        ).start()
        ...
        mock.patch(
            ...
            lambda *a: self._mocked_call("GetChangeComments", *a),
        ).start()
        mock.patch(
            "git_cl.gerrit_util.AddReviewers",
            lambda *a: self._mocked_call("AddReviewers", *a),
        ).start()
        mock.patch(
            "git_cl.gerrit_util.SetReview",
            lambda h, i, msg=None, labels=None, notify=None, ready=None, automatic_attention_set_update=None, project=None: (
                self._mocked_call(
                    "SetReview",
                    h,
                    i,
                    ...
                    notify,
                    ready,
                    automatic_attention_set_update,
                    project,
                )
            ),
        ).start()
        mock.patch(
            "git_cl.gerrit_util.CreateDraft",
            lambda h, i, revision="current", body=None: self._mocked_call(
                "CreateDraft", h, i, revision, body
            ),
        ).start()
        mock.patch(
            "git_cl.gerrit_util.LuciContextAuthenticator.is_applicable",
            return_value=False,
        ).start()
        mock.patch(
            "git_cl.gerrit_util.GceAuthenticator.is_applicable",
            return_value=False,
        ).start()
        mock.patch(
            "git_cl.gerrit_util.ValidAccounts",
            lambda *a: self._mocked_call("ValidAccounts", *a),
        ).start()
        mock.patch("sys.exit", side_effect=SystemExitMock).start()
        ...
        mock.patch(
            ...
            return_value=("origin", "refs/heads/main"),
        ).start()
        mock.patch(
            "scm.GIT.CaptureStatus", return_value=[("M", "foo.txt")]
        ).start()
    ...
    def test_ask_for_explicit_yes_true(self):
        ...
        self.assertEqual(
            "prompt [Yes/No]: Please, type yes or no: ", sys.stdout.getvalue()
        )
    ...
    def test_LoadCodereviewSettingsFromFile_format_js(self):
        codereview_file = io.StringIO("FORMAT_JS: True")
        self.calls = []
        git_cl.LoadCodereviewSettingsFromFile(codereview_file)
        self.assertEqual("True", scm.GIT.GetConfig("", "cl.format-js"))
    ...
    def _gerrit_upload_calls(
        ...
        calls += [
            ...
            (
                (["git", "rev-list", parent + ".." + ref_to_push],),
                "1hashPerLine\n",
            ),
        ]
        ...
        if not title and squash_mode != "override_nosquash":
            if issue is None:
                ...
                title = "User input"
        if title:
            ref_suffix_list.append(
                "m=" + gerrit_util.PercentEncodeForGitRef(title)
            )
            ...
            calls += [
                (
                    (
                        ...
                        sorted(reviewers)
                        + ["joe@example.com", "test-more-cc@chromium.org"]
                        + cc,
                    ),
                    {
                        e: {"email": e}
    ...
    def _run_gerrit_upload_test(
        ...
        mock.patch(
            ...
            lambda _, update: patchset,
        ).start()
        mock.patch(
            "git_cl.gclient_utils.RunEditor",
            lambda *_, **__: self._mocked_call(["RunEditor"]),
        ).start()
        mock.patch(
            "git_cl.DownloadGerritHook",
            lambda force: self._mocked_call("DownloadGerritHook", force),
        ).start()
        mock.patch(
        ...
        mock.patch(
            "git_cl.gclient_utils.FileWrite",
        ...
        mock.patch(
            ...
            return_value=log_description or description,
        ).start()
        mock.patch(
            "git_cl.Changelist._AddChangeIdToCommitMessage",
            return_value=post_amend_description or description,
        ).start()
        mock.patch(
            "git_cl.GenerateGerritChangeId", return_value=change_id
        ).start()
        mock.patch(
            "git_common.get_or_create_merge_base",
            return_value="origin/" + default_branch,
        ).start()
        mock.patch(
            "gerrit_util.GetAccountDetails", getAccountDetailsMock
        ).start()
        mock.patch(
        ...
        scm.GIT.SetConfig(
            "", "branch.main.gerritissue", (str(issue) if issue else None)
        )
        scm.GIT.SetConfig(
            "",
            "remote.origin.url",
        ...
        if squash_mode == "override_nosquash":
            if issue:
                mock.patch(
                    "gerrit_util.GetChange", return_value={"_number": issue}
                ).start()
            else:
        ...
        if fetched_status == "ABANDONED" or (
            ...
            if fetched_status == "MERGED" and reset_issue:
                ...
                issue = None
            mock.patch(
                "gclient_utils.temporary_file", TemporaryFileMock()
            ).start()
        ...
        if squash:
            ...
            self.assertEqual(
                ref_to_push,
                scm.GIT.GetBranchConfig(
                    "", "main", git_cl.GERRIT_SQUASH_HASH_CONFIG_KEY
                ),
            )
    ...
    def test_upload_all_squashed_cherry_pick(
        self,
        mockCherryPickCommit,
        ...
        *_mocks,
        ...
        scm.GIT.SetConfig(
            "",
            "branch.upstream-branch.gerritsquashhash",
        ...
        # Asserts
        mockCherryPickCommit.assert_called_once_with(
            options, upstream_gerrit_commit
        )
        expected_refspec = (
            "commit-to-push:refs/for/refs/heads/main%"
            "m=honk_stonk,topic=circus,hashtag=cow"
        )
        expected_refspec_opts = ["m=honk_stonk", "topic=circus", "hashtag=cow"]
        mockRunGitPush.assert_called_once_with(
            expected_refspec,
    ...
    def test_upload_all_squashed(
        ...
        *_mocks,
        ...
        expected_refspec = (
            "commit-to-push:refs/for/refs/heads/main%topic=circus,hashtag=cow"
        )
    ...
    def test_upload_all_squashed_single_cl_reviewer_update(
        ...
        # Reviewers and CCs are folded into push options with :silent, avoiding separate AddReviewers REST call.
        mockAddReviewers.assert_not_called()
        mockRunGitPush.assert_called_once()
        refspec_arg = mockRunGitPush.call_args[0][0]
        push_options_arg = mockRunGitPush.call_args[0][3]
        self.assertIn("m=stonks", refspec_arg)
        self.assertNotIn("notify=NONE", refspec_arg)
        self.assertNotIn("r=r1@google.com", refspec_arg)
        self.assertIn("r=r1@google.com:silent", push_options_arg)
        self.assertIn("cc=c1@google.com:silent", push_options_arg)
        self.assertEqual(new_upload_current.reviewers, [])
        self.assertEqual(new_upload_current.ccs, [])
    ...
    @mock.patch("git_cl._UploadAllPrecheck")
    @mock.patch("git_cl.Changelist.PrepareSquashedCommit")
    def test_upload_all_squashed_single_cl_reviewer_fallback(
        self,
        mockSquashedCommit,
        mockUploadAllPrecheck,
        mockRunGitPush,
        ...
        new_upload_current = git_cl._NewUpload(
            ["r1@google.com", "ldap:invalid_reviewer", "user~name@domain.com"],
            ["ldap:invalid_cc", "group/team"],
            "commit-to-push",
            "new-last-upload",
            "next-upstream-ancestor",
            change_desc,
        ...
        refspec_arg = mockRunGitPush.call_args[0][0]
        push_options_arg = mockRunGitPush.call_args[0][3]
        # r1@google.com was pushed with :silent in push options
        self.assertNotIn("r=r1@google.com", refspec_arg)
        self.assertIn("r=r1@google.com:silent", push_options_arg)
        self.assertNotIn("ldap:invalid_reviewer", push_options_arg)
        self.assertNotIn("user~name@domain.com", push_options_arg)
        self.assertNotIn("ldap:invalid_cc", push_options_arg)
        self.assertNotIn("group/team", push_options_arg)
        # Non-standard identifiers fell back to AddReviewers REST
        mockAddReviewers.assert_called_once_with(
            "chromium-review.googlesource.com",
            "project~1234",
            reviewers=["ldap:invalid_reviewer", "user~name@domain.com"],
            ccs=["ldap:invalid_cc", "group/team"],
            notify=False,
        )
        self.assertEqual(
            new_upload_current.reviewers,
            ["ldap:invalid_reviewer", "user~name@domain.com"],
        )
        self.assertEqual(
            new_upload_current.ccs, ["ldap:invalid_cc", "group/team"]
        )
    ...
    @mock.patch("git_cl._UploadAllPrecheck")
    @mock.patch("git_cl.Changelist.PrepareSquashedCommit")
    def test_upload_all_squashed_single_cl_reviewer_send_mail(
        self,
        mockSquashedCommit,
        mockUploadAllPrecheck,
        mockRunGitPush,
        ...
        refspec_arg = mockRunGitPush.call_args[0][0]
        push_options_arg = mockRunGitPush.call_args[0][3]
        # When send_mail is True, ready and notify=ALL are set in refspec
        self.assertIn("ready", refspec_arg)
        self.assertIn("notify=ALL", refspec_arg)
        self.assertNotIn("notify=NONE", refspec_arg)
        self.assertNotIn("r=r1@google.com", refspec_arg)
        # Reviewers and CCs are passed as push options without :silent
        self.assertIn("r=r1@google.com", push_options_arg)
        self.assertIn("cc=c1@google.com", push_options_arg)
        self.assertNotIn("r=r1@google.com:silent", push_options_arg)
        self.assertNotIn("cc=c1@google.com:silent", push_options_arg)
        self.assertEqual(new_upload_current.reviewers, [])
        self.assertEqual(new_upload_current.ccs, [])
    ...
    @mock.patch("git_cl._UploadAllPrecheck")
    @mock.patch("git_cl.Changelist.PrepareSquashedCommit")
    def test_upload_all_squashed_single_cl_reviewer_dedup_and_disjoint(
        self,
        mockSquashedCommit,
        mockUploadAllPrecheck,
        mockRunGitPush,
        ...
        new_upload_current = git_cl._NewUpload(
            ["r1@google.com", "r1@google.com", "r2@google.com"],
            ["r1@google.com", "c1@google.com", "c1@google.com"],
            "commit-to-push",
            "new-last-upload",
            "next-upstream-ancestor",
            change_desc,
        ...
        mockAddReviewers.assert_not_called()
        push_options_arg = mockRunGitPush.call_args[0][3]
        # Reviewers deduplicated with :silent
        self.assertEqual(push_options_arg.count("r=r1@google.com:silent"), 1)
        self.assertEqual(push_options_arg.count("r=r2@google.com:silent"), 1)
        # CCs deduplicated and r1@google.com removed from CCs because it is a reviewer
        self.assertEqual(push_options_arg.count("cc=c1@google.com:silent"), 1)
        self.assertNotIn("cc=r1@google.com:silent", push_options_arg)
        self.assertEqual(new_upload_current.reviewers, [])
        self.assertEqual(new_upload_current.ccs, [])
    ...
    @mock.patch("git_cl._UploadAllPrecheck")
    @mock.patch("git_cl.Changelist.PrepareSquashedCommit")
    def test_upload_all_squashed_single_cl_reviewer_push_failure_preserves_state(
        self,
        mockSquashedCommit,
        mockUploadAllPrecheck,
        mockRunGitPush,
        ...
        mockSquashedCommit.side_effect = [new_upload_current]
        mockRunGitPush.side_effect = Exception("Push failed: network error")
        mockGerritChangeIdentifier.return_value = "project~1234"
        options = optparse.Values()
        options.send_mail = options.private = False
        options.squash = True
        ...
        # Reviewers and CCs must remain intact in new_upload_current if push failed
        self.assertEqual(new_upload_current.reviewers, ["r1@google.com"])
        self.assertEqual(new_upload_current.ccs, ["c1@google.com"])
    ...
    @mock.patch("git_cl._UploadAllPrecheck")
    @mock.patch("git_cl.Changelist.PrepareSquashedCommit")
    def test_upload_all_squashed_single_cl_reviewer_silent_rejected_retry_fallback(
        self,
        mockSquashedCommit,
        mockUploadAllPrecheck,
        mockRunGitPush,
        ...
        mockSquashedCommit.side_effect = [new_upload_current]
        called_proc_err = subprocess2.CalledProcessError(
            1,
            ["git", "push"],
            "cwd",
            b"remote: error: reviewer r1@google.com:silent not found",
            b"",
        )
        mockRunGitPush.side_effect = [
            git_cl.GitPushError(
                "Failed to create a change", push_error=called_proc_err
            ),
            "remote: https://chromium-review.googlesource.com/1234\n",
        ]
        mockGerritChangeIdentifier.return_value = "project~1234"
        ...
        git_cl.UploadAllSquashed(options, [])
        self.assertEqual(mockRunGitPush.call_count, 2)
        first_push_options = mockRunGitPush.call_args_list[0][0][3]
        second_push_options = mockRunGitPush.call_args_list[1][0][3]

        # First push contained :silent options
        self.assertIn("r=r1@google.com:silent", first_push_options)
        self.assertIn("cc=c1@google.com:silent", first_push_options)
        # Second push stripped :silent options
        self.assertNotIn("r=r1@google.com:silent", second_push_options)
        self.assertNotIn("cc=c1@google.com:silent", second_push_options)

        # Fallback to AddReviewers REST
        mockAddReviewers.assert_called_once_with(
            "chromium-review.googlesource.com",
            "project~1234",
    ...
    @mock.patch("git_cl._UploadAllPrecheck")
    @mock.patch("git_cl.Changelist.PrepareSquashedCommit")
    def test_upload_all_squashed_single_cl_reviewer_not_found_send_mail_retry_fallback(
        self,
        mockSquashedCommit,
        mockUploadAllPrecheck,
        mockRunGitPush,
        ...
        called_proc_err = subprocess2.CalledProcessError(
            ...
            ["git", "push"],
            "cwd",
            b"remote: error: Account 'r1@google.com' not found",
            b"",
        )
        mockRunGitPush.side_effect = [
            git_cl.GitPushError(
        ...
        # First push contained non-silent options because send_mail=True
        self.assertIn("r=r1@google.com", first_push_options)
        self.assertIn("cc=c1@google.com", first_push_options)
        # Second push stripped folded reviewer/CC options
        self.assertNotIn("r=r1@google.com", second_push_options)
        self.assertNotIn("cc=c1@google.com", second_push_options)
    ...
    @mock.patch("git_cl._UploadAllPrecheck")
    @mock.patch("git_cl.Changelist.PrepareSquashedCommit")
    def test_upload_all_squashed_single_cl_standard_push_error_no_retry(
        self,
        mockSquashedCommit,
        mockUploadAllPrecheck,
        mockRunGitPush,
        ...
        mockSquashedCommit.side_effect = [new_upload_current]
        called_proc_err = subprocess2.CalledProcessError(
            1, ["git", "push"], "cwd", b"remote: error: branch protected", b""
        )
        mockRunGitPush.side_effect = git_cl.GitPushError(
            "Failed to create a change", push_error=called_proc_err
        )
        mockGerritChangeIdentifier.return_value = "project~1234"
        options = optparse.Values()
        ...
        # Standard push errors should fail immediately and NOT retry
        self.assertEqual(mockRunGitPush.call_count, 1)
        mockAddReviewers.assert_not_called()
    ...
    @mock.patch(
        "git_cl.Changelist.GetGerritHost",
        return_value="webrtc-review.googlesource.com",
    )
    @mock.patch(
        "git_cl.Changelist.GetRemoteBranch",
        return_value=("origin", "refs/remotes/origin/main"),
    ...
    @mock.patch("git_cl._UploadAllPrecheck")
    @mock.patch("git_cl.Changelist.PrepareSquashedCommit")
    def test_upload_all_squashed_single_cl_non_chromium_host_no_valid_accounts(
        self,
        mockSquashedCommit,
        mockUploadAllPrecheck,
        mockRunGitPush,
        ...
        mockSquashedCommit.side_effect = [new_upload_current]
        mockRunGitPush.return_value = (
            "remote: https://webrtc-review.googlesource.com/1234\n"
        )
        mockGerritChangeIdentifier.return_value = "project~1234"
        options = optparse.Values()
        options.send_mail = options.private = False
        ...
        # Non-Chromium hosts must fold into push options without calling ValidAccounts or AddReviewers REST
        mockValidAccounts.assert_not_called()
        mockAddReviewers.assert_not_called()
        refspec_arg = mockRunGitPush.call_args[0][0]
        push_options_arg = mockRunGitPush.call_args[0][3]
        self.assertIn("m=stonks", refspec_arg)
        self.assertNotIn("notify=NONE", refspec_arg)
        self.assertIn("r=r1@webrtc.org:silent", push_options_arg)
        self.assertIn("cc=c1@webrtc.org:silent", push_options_arg)
    ...
    @mock.patch("git_cl._UploadAllPrecheck")
    @mock.patch("git_cl.Changelist.PrepareSquashedCommit")
    def test_upload_all_squashed_single_cl_reviewer_preserves_custom_notify(
        self,
        mockSquashedCommit,
        mockUploadAllPrecheck,
        mockRunGitPush,
        ...
        options.hashtags = []
        options.target_branch = None
        options.push_options = ["notify=OWNERS"]
        ...
        self.assertNotIn("notify=NONE", refspec_arg)
        self.assertNotIn("r=r1@google.com", refspec_arg)
        # Push options are passed as the 4th argument to _RunGitPushWithTraces
        self.assertIn("notify=OWNERS", push_options_arg)
        self.assertIn("r=r1@google.com:silent", push_options_arg)
        self.assertIn("cc=c1@google.com:silent", push_options_arg)
    ...
    @mock.patch("git_cl._UploadAllPrecheck")
    @mock.patch("git_cl.Changelist.PrepareSquashedCommit")
    def test_upload_all_squashed_single_cl_reviewer_regex_edge_cases(
        self,
        mockSquashedCommit,
        mockUploadAllPrecheck,
        mockRunGitPush,
        ...
        refspec_arg = mockRunGitPush.call_args[0][0]
        push_options_arg = mockRunGitPush.call_args[0][3]
        self.assertNotIn("user+tag@example.com", refspec_arg)
        # Valid emails with +, -, and subdomains are pushed in push_options with :silent
        self.assertIn("r=user+tag@example.com:silent", push_options_arg)
        self.assertIn("r=user-name@domain-name.com:silent", push_options_arg)
        self.assertIn("cc=user@sub.domain.co.uk:silent", push_options_arg)
        # Comma, consecutive dots in local part, and double dots in domain must NOT be in push_options
        self.assertNotIn("user,comma@example.com:silent", push_options_arg)
        self.assertNotIn("user..name@example.com:silent", push_options_arg)
        self.assertNotIn("user@domain..com:silent", push_options_arg)
        # Invalid / non-safe entries fall back to AddReviewers REST
        mockAddReviewers.assert_called_once_with(
            "chromium-review.googlesource.com",
            "project~1234",
            reviewers=["user,comma@example.com", "user..name@example.com"],
            ccs=["user@domain..com"],
            notify=False,
        )
        self.assertEqual(
            new_upload_current.reviewers,
            ["user,comma@example.com", "user..name@example.com"],
        )
        self.assertEqual(new_upload_current.ccs, ["user@domain..com"])
    ...
    @mock.patch("git_cl._UploadAllPrecheck")
    @mock.patch("git_cl.Changelist.PrepareSquashedCommit")
    def test_upload_all_squashed_no_space(
        self,
        mockSquashedCommit,
        mockUploadAllPrecheck,
        mockRunGitPush,
    ...
    @mock.patch("git_cl._UploadAllPrecheck")
    @mock.patch("git_cl.Changelist.PrepareSquashedCommit")
    def test_upload_all_squashed_count_mismatch(
        self,
        mockSquashedCommit,
        mockUploadAllPrecheck,
        mockRunGitPush,
        ...
        *_mocks,
        ...
        with self.assertRaises(SystemExitMock):
            git_cl.UploadAllSquashed(options, orig_args)
        self.assertIn(
            "Created|Updated 1 issues on Gerrit, but 2 expected.",
            sys.stderr.getvalue(),
        )
        self.assertIn(
            "Detected change numbers: ['1233']", sys.stderr.getvalue()
        )
    ...
    @mock.patch("git_cl._UploadAllPrecheck")
    @mock.patch("git_cl.Changelist.PrepareSquashedCommit")
    def test_upload_all_squashed_external_changes(
        self,
        mockSquashedCommit,
        mockUploadAllPrecheck,
        mockRunGitPush,
    ...
    def test_upload_all_precheck_long_chain(
        ...
        *_mocks,
        ...
        mockGetRemoteBranch.return_value = (
            "origin",
        ...
        mockFetchUpstreamTuple.side_effect = [
            (".", "refs/heads/upstream3"),
            (".", "refs/heads/blank3"),
            (".", "refs/heads/blank2"),
    ...
    def getDescriptionForUploadTest(
        ...
        mockBatchListOwners.side_effect = lambda ps: {
            p: owners_by_path.get(p) for p in ps
        }
    ...
    def test_get_hash_tags(self):
        ...
        self.assertEqual(
            ...
            git_cl.GetTargetRef(
                "origin", "refs/remotes/branch-heads/123", None
            ),
        )
        self.assertEqual(
            "refs/diff/test",
            git_cl.GetTargetRef(
                "origin", "refs/remotes/origin/refs/diff/test", None
            ),
        )
        self.assertEqual(
            "refs/heads/chrome/m42",
            git_cl.GetTargetRef(
                "origin", "refs/remotes/origin/chrome/m42", None
            ),
        )
        ...
        for branch in (
            "branch-heads/123",
            "remotes/branch-heads/123",
            "refs/remotes/branch-heads/123",
        ):
            self.assertEqual(
                "refs/branch-heads/123",
                git_cl.GetTargetRef(
                    "origin", "refs/remotes/origin/main", branch
                ),
            )
        for branch in (
            "origin/main",
            ...
            self.assertEqual(
                "refs/heads/main",
                git_cl.GetTargetRef(
                    "origin", "refs/remotes/branch-heads/123", branch
                ),
            )
    ...
    def test_patch_gerrit_no_commit(self):
        ...
        self.assertEqual(
            "123456", scm.GIT.GetBranchConfig("", "main", "gerritissue")
        )
        self.assertEqual(
            "7", scm.GIT.GetBranchConfig("", "main", "gerritpatchset")
        )
    ...
    def test_patch_gerrit_new_branch_fails(self):
        ...
        with mock.patch("git_new_branch.create_new_branch", return_value=1):
            self.assertEqual(
                git_cl.main(["patch", "-b", "feature", "123456"]), 1
            )
            ...
            (
                (
                    [
                        "git",
                        "fetch",
                        "https://else.googlesource.com/my/repo",
    ...
    def _setup_mock_for_cookies_authenticator(self, creds):
        ...
        self._add_patch_with_cleanup(
            "git_cl.gerrit_util._Authenticator.get",
            cookies_authenticator_factory,
        )
        ...
        self.calls = [
            (
                (
                    ...
                    "%(remote)s",
                    {"branch": "main", "remote": "custom-scheme://repo"},
                ),
                None,
            ),
        ]
    ...
    def test_gerrit_ensure_authenticated_non_url(self):
        scm.GIT.SetConfig(
            "", "remote.origin.url", "git@somehost.example:foo/bar.git"
        )
        self.calls = [
            (
                (
        ...
        mock.patch(
            "logging.error", lambda *a: self._mocked_call("logging.error", *a)
        ).start()
    ...
    def test_gerrit_ensure_authenticated_with_reauth(self):
        ...
        self._add_patch_with_cleanup(
            "git_cl.gerrit_util.ensure_authenticated", mock_ensure_authenticated
        )
    ...
    def test_StatusFieldOverrideIssueMissingArgs(self):
        try:
            ...
            self.assertIn(
                "--field must be given when --issue is set.",
                sys.stderr.getvalue(),
            )
    ...
    def test_description_appends_bug_line(self):
        ...
        mock.patch(
            "git_cl.Changelist.FetchDescription", lambda *args: current_desc
        ).start()
        mock.patch(
            "git_cl.Changelist.UpdateDescription", UpdateDescription
        ).start()
    ...
    def test_description_set_stdin(self):
        mock.patch("git_cl.Changelist", ChangelistMock).start()
        mock.patch(
            "git_cl.sys.stdin", io.StringIO("hi \r\n\t there\n\nman")
        ).start()
    ...
    def test_diff_with_files(self):
        ...
        with mock.patch(
            "git_cl.OptionParser.error", side_effect=ParserErrorMock
        ):
    ...
    def test_GerritCmdLand(self):
        ...
        scm.GIT.SetConfig(
            "", "branch.main.gerritserver", "chromium-review.googlesource.com"
        )
        self.calls += [
    ...
    def test_print_current_creds(self):
        ...
        mock.patch(
            "git_cl.gerrit_util.CookiesAuthenticator", CookiesAuthenticatorMock
        ).start()
        git_cl._GitCookiesChecker().print_current_creds()
        self.assertEqual(
            list(sys.stdout.getvalue().splitlines()),
            [
    ...
    def _common_creds_check_mocks(self):
        def exists_mock(path):
            ...
            if base == ".gitcookies":
                return self._mocked_call(
                    "os.path.exists", os.path.join(dirname, base)
                )
            ...
            "messages": [
                {
                    ...
                    "author": {
                        "_account_id": 1111084,
                        "email": "could-be-anything@example.com",
                        "name": "LUCI CQ",
                    },
                    "date": "2017-03-15 20:08:45.000000000",
                    "id": "f5a6c25ecbd3b3b54a43ae418ed97eff046dc50b",
                ...
                },
                {
                    "_revision_number": 2,
                    "author": {
                        "_account_id": 11151243,
                        "email": "owner@example.com",
                        "name": "owner",
                    },
                    "date": "2017-03-16 20:00:41.000000000",
                    "id": "f5a6c25ecbd3b3b54a43ae418ed97eff046d1234",
                    "message": "PTAL",
                },
                {
                    "_revision_number": 2,
                    ...
                    "date": "2017-03-17 05:19:37.500000000",
                    "id": "f5a6c25ecbd3b3b54a43ae418ed97eff046d4568",
                    "message": "Patch Set 2: Code-Review+1",
                },
                {
                    "_revision_number": 2,
                    "author": {"_account_id": 42, "name": "reviewer"},
                    "date": "2017-03-17 05:19:37.900000000",
                    "id": "f5a6c25ecbd3b3b54a43ae418ed97eff046d0000",
                    "message": "A bot with no email set",
                },
            ],
        }
        self.calls = [
                    ...
                    "/COMMIT_MSG": [
                        {
                            ...
                            "updated": "2017-03-17 05:19:37.500000000",
                            "patch_set": 2,
                            "side": "REVISION",
                    ...
                    "codereview.settings": [
                        {
                            ...
                            "updated": "2017-03-16 20:00:41.000000000",
                            "patch_set": 2,
                            "side": "PARENT",
                            "line": 42,
        ...
        expected_comments_summary = [
            git_cl._CommentSummary(
                message=(
                    ...
                    + "\n"
                    + "codereview.settings\n"
                    + "  Base, Line 42: https://crrev.com/c/1/2/"
                    + "codereview.settings#b42 (resolved)\n"
                    + "  I removed this because it is bad\n\n"
                    + "  Base, Line 42: https://crrev.com/c/1/2/"
                    + "codereview.settings#b42 (resolved)\n"
                    + "  And another thing\n"
                ),
            ...
            git_cl._CommentSummary(
                message=(
                    ...
                    + "\n"
                    + "/COMMIT_MSG\n"
                    + "  PS2, File comment: https://crrev.com/c/1/2/"
                    "/COMMIT_MSG# (resolved)\n"
                    + "  Please include a bug link\n"
                ),
                message_json={
                    "message": "Patch Set 2: Code-Review+1",
                    "comments": [
    ...
    def test_get_remote_url_non_existing_mirror(self):
        ...
        scm.GIT.SetConfig(
            "", "remote.origin.url", "/cache/this-dir-doesnt-exist"
        )
        self.calls = [
...
class ChangelistTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        mock.patch(
            ...
            return_value="https://chromium-review.googlesource.com",
        ).start()
        ...
        mock.patch(
            "git_cl.Changelist.GetGerritProject", return_value="project"
        ).start()
        mock.patch("sys.exit", side_effect=SystemExitMock).start()
    ...
    def testDefaultTitleEmptyMessage(self):
        ...
        for user_title in ["", "y", "Y"]:
            self.assertEqual(
                cl._GetTitleForUpload(options), self.LAST_COMMIT_SUBJECT
            )
    ...
    def testPrepareSquashedCommit(self, mockPrepareChange, mockRunGit, *_mocks):
        ...
        def mock_run_git(commands):
            if {"commit-tree", tree_hash, "-p", parent_hash, "-F"}.issubset(
                set(commands)
            ):
                return hash_to_push
            if {
        ...
        new_upload = cl.PrepareSquashedCommit(
            options, parent_hash, parent_orig_hash
        )
        ...
        mockPrepareChange.assert_called_with(
            options, parent_orig_hash, end_hash
        )
    ...
    @mock.patch("git_cl.Changelist._PrepareChange")
    @mock.patch("git_cl.Changelist.GetCommonAncestorWithUpstream")
    def testPrepareCherryPickSquashedCommit(
        self,
        mockGetCommonAncestorWithUpstream,
        mockPrepareChange,
        mockRunGit,
        ...
        *_mocks,
        ...
        new_upload = cl.PrepareCherryPickSquashedCommit(
            options, upstream_gerrit_hash
        )
        self.assertEqual(new_upload.reviewers, reviewers)
        self.assertEqual(new_upload.ccs, ccs)
        self.assertEqual(new_upload.commit_to_push, hash_to_push)
        self.assertEqual(
            new_upload.new_last_uploaded_commit, hash_to_save_as_last_upload
        )
        self.assertEqual(new_upload.change_desc, change_desc)
    ...
    def testPrepareChange_new(
        ...
        *_mocks,
        ...
        cl = git_cl.Changelist()
        reviewers, ccs, change_desc = cl._PrepareChange(
            options, parent, latest_tree
        )
        self.assertEqual(reviewers, ["horse@apple.farm"])
        self.assertEqual(
            ccs, ["cow@moo.farm", "chicken@bok.farm", "cow2@moo.farm"]
        )
        self.assertEqual(
            change_desc._description_lines,
            [
                "AH!",
                "CC=cow2@moo.farm",
                "R=horse@apple.farm",
        ...
        mockRunHook.assert_called_once_with(
            committing=False,
            may_prompt=True,
            verbose=False,
            parallel=False,
    ...
    @mock.patch("git_cl.gerrit_util.AddReviewers")
    def testPostUploadUpdates(
        self, mockAddReviewers, mockRunPostHook, mockSetPatchset, *_mocks
    ):
        ...
        mockRunPostHook.assert_called_once_with(
            True, "parent-commit", change_desc.description
        )
    ...
    def testGerritURLRepoIdentity(self):
        ...
        for x, want in cases:
            with self.subTest(c=x):
                self.assertEqual(
                    git_cl.Changelist._GerritURLRepoIdentity(x), want
                )
...
class CMDTestCaseBase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        mock.patch(
            "git_cl.Changelist.GetMostRecentPatchset", return_value=7
        ).start()
        mock.patch(
            "git_cl.Changelist.GetMostRecentDryRunPatchset", return_value=6
        ).start()
        mock.patch(
        ...
        mock.patch(
            "auth.Authenticator", return_value=AuthenticatorMock()
        ).start()
        mock.patch("git_cl.subprocess2.call", return_value=0).start()
        mock.patch(
            "gerrit_util.GetChangeDetail", return_value=self._CHANGE_DETAIL
        ).start()
        mock.patch(
...
class CMDPresubmitTestCase(CMDTestCaseBase):
    ...
    def setUp(self):
        ...
        mock.patch(
            "git_cl.Changelist.FetchDescription",
            return_value="fetch description",
        ).start()
        mock.patch(
            "git_cl._create_description_from_log",
            return_value="get description",
        ).start()
        mock.patch(
    ...
    def testNoParallelOption(self):
        self.assertEqual(0, git_cl.main(["presubmit", "--no-parallel"]))
        git_cl.Changelist.RunHook.assert_called_once_with(
            committing=True,
            may_prompt=False,
            verbose=0,
...
class CMDTryResultsTestCase(CMDTestCaseBase):
    _DEFAULT_REQUEST = {
        ...
        "fields": (
            "builds.*.id,builds.*.builder,builds.*.status"
    ...
    def test_parse_bucket(self):
        ...
        for test_case in test_cases:
            ...
            if test_case.get("has_warning"):
                expected_warning = (
                    "WARNING Please use %s/%s to specify the bucket"
                    % (test_case["result"])
                )
...
class MakeRequestsHelperTestCase(unittest.TestCase):
    ...
    def testMakeRequestsHelperPresubmitSetsDryRunProperty(self):
        ...
        self.assertEqual(
            requests[0]["properties"],
            {"category": "git_cl_try", "dry_run": "true"},
        )
    ...
    def testMakeRequestsHelperCategorySet(self):
        ...
        self.assertEqual(
            requests[0]["properties"], {"category": "my-special-category"}
        )
...
class CMDOwnersTestCase(CMDTestCaseBase):
    def setUp(self):
        ...
        mock.patch(
            ...
            return_value=list(self.owners_by_path),
        ).start()
        mock.patch(
            "git_cl.Changelist.GetCommonAncestorWithUpstream",
            return_value="upstream",
        ...
        mock.patch(
            "git_cl.Changelist.GetGerritHost", return_value="host"
        ).start()
        mock.patch(
            "git_cl.Changelist.GetGerritProject", return_value="project"
        ).start()
        ...
        mock.patch(
            ...
            return_value=self.owners_by_path,
        ).start()
        mock.patch(
    ...
    def testNoIssueNumber(self, _mock_get_issue, mock_owners_finder):
        ...
        mock_owners_finder.assert_called_once_with(
            ["foo", "bar"],
            "author",
            [],
            mock.ANY,
...
class CMDLintTestCase(CMDTestCaseBase):
    ...
    def testLintSingleFile(self, *_mock):
        ...
        self.assertIn(
            "pdf.h:3:  (cpplint) Do not indent within a namespace",
    ...
    def testLintMultiFiles(self, *_mock):
        ...
        self.assertIn(
            "pdf.cc:3:  (cpplint) Do not indent within a namespace",
            git_cl.sys.stderr.getvalue(),
        )
    ...
    def testLintChangelist(self, *_mock):
        ...
        self.assertIn(
            "chg-1.h:3:  (cpplint) Do not indent within a namespace",
            git_cl.sys.stderr.getvalue(),
        )
        self.assertIn(
            "chg-2.cc:3:  (cpplint) Do not indent within a namespace",
            git_cl.sys.stderr.getvalue(),
        )
...
"""
        self.assertEqual(
            git_cl._create_commit_message(orig_message), expected_message
        )
...
"""
        self.assertEqual(
            git_cl._create_commit_message(orig_message, bug), expected_message
        )
    ...
    def testCherryPick_AllowConflicts(
        self, _mockGetUrl, mockCherryPick, mockQueryChanges
    ):
        mockQueryChanges.return_value = [
            {
                "id": "change_id",
                "revisions": {"abc": {"commit": {"message": "msg"}}},
            }
        ]
        ...
        self.assertEqual(
            0,
            git_cl.main(
                ["cherry-pick", "--branch", "main", "abc", "--allow-conflicts"]
            ),
        )
    ...
    @mock.patch("gerrit_util.CherryPick")
    @mock.patch("gerrit_util.GetChangePageUrl", return_value="url")
    def testCherryPick_NoAllowConflicts(
        self, _mockGetUrl, mockCherryPick, mockQueryChanges
    ):
        mockQueryChanges.return_value = [
            {
    ...
    @mock.patch("gerrit_util.CherryPick")
    @mock.patch("gerrit_util.GetChangePageUrl", return_value="url")
    def testCherryPick_ConflictPromptYes(
        self, _mockGetUrl, mockCherryPick, mockQueryChanges, mockAskForData
    ):
        mockQueryChanges.return_value = [
            {
                "id": "change_id",
    ...
    @mock.patch("gerrit_util.CherryPick")
    @mock.patch("gerrit_util.GetChangePageUrl", return_value="url")
    def testCherryPick_ConflictPromptNo(
        self, _mockGetUrl, mockCherryPick, mockQueryChanges, mockAskForData
    ):
        mockQueryChanges.return_value = [
            {
    ...
    @mock.patch("gerrit_util.AddReviewers")
    @mock.patch("gerrit_util.GetChangePageUrl", return_value="url")
    def testCherryPick_WithMilestone(
        self, _mockGetUrl, mockAddReviewers, mockCherryPick, mockQueryChanges
    ):
        mockQueryChanges.return_value = [
            {
                "id": "change_id",
    ...
    @mock.patch("gerrit_util.AddReviewers")
    @mock.patch("gerrit_util.GetChangePageUrl", return_value="url")
    def testCherryPick_WithRubberStamper(
        self, _mockGetUrl, mockAddReviewers, mockCherryPick, mockQueryChanges
    ):
        mockQueryChanges.return_value = [
            {
...
class CMDSplitTestCase(CMDTestCaseBase):
    def setUp(self):
        super(CMDTestCaseBase, self).setUp()
        self.mock_get_root = mock.patch(
            "git_cl.Settings.GetRoot", return_value="root"
        ).start()
        self.mock_split_cl = mock.patch(
            "split_cl.SplitCl", return_value=0
        ).start()
    ...
    def testDescriptionFlagRequired(self):
        ...
        self.assertEqual(
            git_cl.main(["split", "--description=SomeFile.txt"]), 0
        )
    ...
    def testTargetRangeParsing(self):
        ...
        def ensureFailure(flags):
            self.assertRaises(
                ParserErrorMock, git_cl.main, (["split", "-n"] + flags)
            )
...
class BuildbucketBatchTest(unittest.TestCase):
    ...
    def test_buildbucket_batch_error(self, mock_comm):
        mock_comm.return_value = (
            ('{"responses": [{"error": {"message": "bot fail"}}]}', ""),
            1,
        )
    ...
    def test_buildbucket_search_success(self, mock_comm):
        mock_comm.return_value = (
            (
                '{"responses": [{"searchBuilds": {"builds": [{"id": "123"}]}}]}',
                "",
            ),
            0,
        )
    ...
    def test_buildbucket_search_error(self, mock_comm):
        mock_comm.return_value = (
            ('{"responses": [{"error": {"message": "search fail"}}]}', ""),
            1,
        )
        with self.assertRaises(git_cl.BuildbucketResponseException) as cm:
...
class TestFindGitDir(unittest.TestCase):
    ...
    def test_find_git_dir_worktree_relative(self):
        ...
        self.assertEqual(
            git_cl.FindGitDir(wt_root), os.path.abspath(target_git_dir)
        )
