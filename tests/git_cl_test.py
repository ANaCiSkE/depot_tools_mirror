...
class CMDFormatTestCase(unittest.TestCase):
    ...
    def testInputDiffFile(self, clang_formatter):
        ...
        try:
            ...
            git_cl.RunCommand.assert_called_with(
                [
                    'vpython3', mock.ANY, '--root', self._top_dir, 'format', 'testing/xvfb_unittest.py',
...
class TestRuffIntegration(unittest.TestCase):
    ...
    def test_run_python_format_ruff_routing(self, mock_run):
        ...
        mock_run.assert_called_once_with(
            ["vpython3", expected_tool, "--root", self.test_dir, "format", "foo.py"],
            cwd=self.test_dir,
    ...
    def test_run_python_format_excluded_file_routed_to_ruff_chromium(
        ...
        expected_tool = os.path.join(depot_tools_path, "ruff_chromium")
        mock_run.assert_called_once_with(
            ["vpython3", expected_tool, "--root", self.test_dir, "format", "excluded.py"],
            cwd=self.test_dir,
            shell=sys.platform == 'win32')
    ...
    def test_run_python_format_ruff_ranges(self, mock_ranges, mock_run):
        ...
        expected_tool = os.path.join(depot_tools_path, "ruff_chromium")
        mock_run.assert_called_once_with([
            "vpython3", expected_tool, "--root", self.test_dir, "format", "foo.py", "--range",
