...
class GClientSmokeGIT(gclient_smoketest_base.GClientSmokeBase):
    def setUp(self):
        ...
        self.env["PATH"] = (
            os.path.join(ROOT_DIR, "testing_support")
            + os.pathsep
            + self.env["PATH"]
        )
    ...
    def testGitmodules_relative(self):
        ...
        self.gclient(
            ["gitmodules"], cwd=self.git_base + os.path.join("repo_19", "dir")
        )

        gitmodules = os.path.join(
            self.git_base, "repo_19", "dir", ".gitmodules"
        )
        with open(gitmodules) as f:
            ...
            self.assertEqual(
                [
                    ...
                    '[submodule "chicken/dickens"]',
                    "\tpath = chicken/dickens",
                    "\turl = /repo_3",
    ...
    def testGitmodules_not_relative(self):
        ...
        self.gclient(
            ["gitmodules"], cwd=self.git_base + os.path.join("repo_20", "foo")
        )

        gitmodules = os.path.join(
            self.git_base, "repo_20", "foo", ".gitmodules"
        )
        with open(gitmodules) as f:
            contents = f.read().splitlines()
            self.assertEqual(
    ...
    def testSync(self):
        ...
        self.parseGclient(
            ["sync", "--deps", "mac", "--jobs", "1"], ["running", "running"]
        )
        tree = self.mangle_git_tree(
            ("repo_1@2", "src"),
            ("repo_2@1", "src/repo2"),
    ...
    def testInstallHooks_no_existing_hook(self):
        ...
        self.assertIn(
            f"{hook} already contains the gclient pre-commit hook.", stdout
        )
    ...
    def testRunHooks(self):
        ...
        out = self.parseGclient(
            ["runhooks", "--deps", "mac"], ["running", "running"]
        )
    ...
    def testRevInfoFilterURL(self):
        ...
        results = self.gclient(
            ["revinfo", "--deps", "mac", "--filter", "%srepo_2" % self.git_base]
        )
    ...
    def testGetConfig(self):
        ...
        self.assertEqual(
            parsed_config["solutions"][0]["custom_deps"],
            {"src/third_party/WebKit": None},
        )
        self.assertEqual(
            parsed_config["solutions"][0]["custom_vars"],
            {"checkout_android": True},
        )
        self.assertEqual(
            parsed_config["solutions"][0]["custom_hooks"],
            [{"name": "my_hook", "action": ["python3", "do_something.py"]}],
        )
        ...
        self.assertIn(
            "Key nonexistent_key not found in .gclient config", stderr
        )
    ...
    def testGetConfigEmpty(self):
        ...
        stdout, stderr, returncode = self.gclient(
            ["getconfig", "target_os"], error_ok=True
        )
        self.assertEqual(2, returncode)
    ...
    def testFlatten(self):
        ...
        self.assertEqual(
            [
                ...
                '    "url": "'
                + self.git_base
                + 'repo_2@%s",' % (self.githash("repo_2", 1)[:7]),
                ...
                "# "
                + self.git_base
                + "repo_2@%s, DEPS" % (self.githash("repo_2", 1)[:7]),
    ...
    def testFlattenPinAllDeps(self):
        ...
        self.assertEqual(
            [
                ...
                '    "url": "'
                + self.git_base
                + 'repo_3@%s",' % (self.githash("repo_3", 2)),
                "    \"condition\": '(repo2_false_var) and (true_str_var)',",
                "  },",
                "",
                '  # "src"',
                ...
                '    "url": "'
                + self.git_base
                + 'repo_6@%s",' % (self.githash("repo_6", 1)),
                "  },",
                "",
                '  # "src" -> "src/mac_repo"',
                '  "src/mac_repo": {',
                '    "url": "'
                + self.git_base
                + 'repo_5@%s",' % (self.githash("repo_5", 3)),
                "    \"condition\": 'checkout_mac',",
                "  },",
                "",
                '  # "src" -> "src/repo8" -> "src/recursed_os_repo"',
                ...
                '    "url": "'
                + self.git_base
                + 'repo_15@%s",' % (self.githash("repo_15", 1)),
                "  },",
                "",
                '  # "src" -> "src/repo16"',
                '  "src/repo16": {',
                '    "url": "'
                + self.git_base
                + 'repo_16@%s",' % (self.githash("repo_16", 1)),
                "  },",
                "",
                '  # "src" -> "src/repo2"',
                '  "src/repo2": {',
                '    "url": "'
                + self.git_base
                + 'repo_2@%s",' % (self.githash("repo_2", 1)),
                "    \"condition\": 'true_str_var',",
                "  },",
                "",
                '  # "src" -> "src/repo4"',
                ...
                '    "url": "'
                + self.git_base
                + 'repo_4@%s",' % (self.githash("repo_4", 2)),
                "    \"condition\": 'False',",
                "  },",
                "",
                '  # "src" -> "src/repo8"',
                ...
                '    "url": "'
                + self.git_base
                + 'repo_8@%s",' % (self.githash("repo_8", 1)),
                "  },",
                "",
                '  # "src" -> "src/unix_repo"',
                '  "src/unix_repo": {',
                ...
                "# "
                + self.git_base
                + "repo_15@%s, DEPS" % (self.githash("repo_15", 1)),
                "# "
                + self.git_base
                + "repo_16@%s, DEPS" % (self.githash("repo_16", 1)),
                "# "
                + self.git_base
                + "repo_2@%s, DEPS" % (self.githash("repo_2", 1)),
                "# "
                + self.git_base
                + "repo_6@%s, DEPS" % (self.githash("repo_6", 1)),
                "# "
                + self.git_base
                + "repo_8@%s, DEPS" % (self.githash("repo_8", 1)),
            ],
            deps_contents.splitlines(),
        )
