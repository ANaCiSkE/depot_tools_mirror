...
class BasicTests(unittest.TestCase):
    ...
    def testGetFirstRemoteUrl(self):
        FAKE_PATH = "/fake/path"
        scm.GIT.SetConfig(
            FAKE_PATH, "remote.origin.url", "first-value", append=True
        )
        scm.GIT.SetConfig(
            FAKE_PATH, "remote.origin.url", "second-value", append=True
        )
    ...
    def testDeleteOrMoveRoot(self):
        ...
        try:
            ...
            self.assertTrue(
                os.path.exists(os.path.join(dest_path, "some_file"))
            )
            self.assertFalse(
                os.path.exists(os.path.join(root_dir, "some_file"))
            )
        finally:
...
"""
    ...
    def CreateGitRepo(git_import, path):
        ...
        try:
            Popen(
                [GIT, "init", "-q"], stdout=PIPE, stderr=STDOUT, cwd=path
            ).communicate()
        except OSError:
        ...
        Popen(
            ...
            stderr=STDOUT,
            cwd=path,
        ).communicate(input=git_import.encode())
        Popen(
            [GIT, "checkout", "-q"], stdout=PIPE, stderr=STDOUT, cwd=path
        ).communicate()
        Popen(
        ...
        Popen(
            [GIT, "checkout", "-b", "new", "origin/main", "-q"],
            stdout=PIPE,
            stderr=STDOUT,
            cwd=path,
        ).communicate()
        Popen(
            [GIT, "push", "origin", "origin/origin:origin/main", "-q"],
            stdout=PIPE,
            stderr=STDOUT,
            cwd=path,
        ).communicate()
        ...
        Popen(
            [GIT, "config", "user.email", "someuser@chromium.org"],
            stdout=PIPE,
            stderr=STDOUT,
            cwd=path,
        ).communicate()
    ...
    def setUp(self):
        ...
        self.enabled = self.CreateGitRepo(
            self.sample_git_import, self.base_path
        )
...
class ManagedGitWrapperTestCase(BaseGitWrapperTestCase):
    ...
    def testRevertNone(self):
        ...
        self.assertEqual(file_list, [])
        self.assertEqual(
            git_wrapper.revinfo(options, self.args, None),
    ...
    def testRevertNew(self):
        ...
        with open(file_path, "w") as f:
            f.writelines("new\n")
        Popen(
            [GIT, "add", "c"], stdout=PIPE, stderr=STDOUT, cwd=self.base_path
        ).communicate()
        file_list = []
        git_wrapper.revert(options, self.args, file_list)
        self.assertEqual(file_list, [file_path])
    ...
    def testStatusRef(self):
        ...
        with open(file_paths[0], "a") as f:
            f.writelines("touched\n")
        git_wrapper = gclient_scm.GitWrapper(
            self.url + "@refs/heads/feature", self.root_dir, self.relpath
        )
    ...
    def testUpdateUpdate(self):
        ...
        self.assertEqual(
            git_wrapper.revinfo(options, (), None),
            "4091c7d010ca99d0f2dd416d4b70b758ae432187",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            git_wrapper._Capture(
                ["config", "--get", "fetch.recurseSubmodules"]
            ),
            "off",
        )
        self.assertEqual(
    ...
    def testUpdateRebase(self):
        ...
        self.assertEqual(
            git_wrapper._Capture(["rev-parse", "HEAD^1"]),
            git_wrapper._Capture(["rev-parse", "origin/main"]),
        )
        sys.stdout.close()
    ...
    def testUpdateUnstagedConflict(self):
        ...
        try:
            ...
            gclient_scm.gclient_utils.Error,
    ...
    def testUpdateLockedBreak(self):
        ...
        git_wrapper.update(options, (), [])
        self.assertRegex(
            sys.stdout.getvalue(), r"breaking lock.*\.git[/|\\]index\.lock"
        )
    ...
    def testRejectsInvalidRevisionOverrideStartingWithHyphen(self):
        ...
        git_wrapper = gclient_scm.GitWrapper(
            f"{self.url}@--upload-pack=touch /tmp/pwn",
            self.root_dir,
            self.relpath,
        )
...
class ManagedGitWrapperTestCaseMock(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.backup_base_path = os.path.join(
            self.root_dir, "old_%s.git" % self.relpath
        )
    ...
    def testGetUsableRevGit(self, mockIsValidRevision):
        ...
        self.assertEqual(
            git_scm.GetUsableRev(self.fake_hash_1, options), self.fake_hash_1
        )
        # An SVN rev with an existing purely git repo should raise an exception.
        self.assertRaises(
            gclient_scm.gclient_utils.Error, git_scm.GetUsableRev, "1", options
        )
    ...
    def testEnsureValidHeadObjectOrCheckout_CouldNotParseHEAD(self, mockClone):
        ...
        mock_capture = mock.Mock(
            side_effect=subprocess2.CalledProcessError(
                ...
                cwd=None,
                stdout=None,
                stderr=b"error: unable to normalize alternate object path...\nfatal: Could not parse object 'HEAD'.",
            )
        )
    ...
    @mock.patch("os.path.exists")
    @mock.patch("git_common.run")
    def testUpdateExistingCheckoutSkipsSnapshotLookup(
        self, mockRun, mockExists, mockIsdir, mockSetFetch, mockBranch
    ):
    ...
    @mock.patch("os.path.exists")
    @mock.patch("git_common.run")
    def testUpdateUrlSwitchUsesNewMirrorDefaultBranch(
        self,
        mockRun,
        mockExists,
        mockIsdir,
    ...
    @mock.patch("os.path.exists")
    @mock.patch("git_common.run")
    def testUpdateMatchingUrlTrustsLocalHead(
        self,
        mockRun,
        mockExists,
        mockIsdir,
        ...
        mockBranch.assert_not_called()
        mockHeadRef.assert_called_once_with(
    ...
    @mock.patch("os.path.exists")
    @mock.patch("git_common.run")
    def testUpdateUpstreamFetchUrlTrustsLocalHead(
        self,
        mockRun,
        mockExists,
        mockIsdir,
...
class UnmanagedGitWrapperTestCase(BaseGitWrapperTestCase):
    ...
    def testUpdateClone(self):
        ...
        git_wrapper = gclient_scm.GitWrapper(
            origin_root_dir, self.root_dir, self.relpath
        )
    ...
    def testUpdateCloneOnCommit(self):
        ...
        self.base_path = join(self.root_dir, self.relpath)
        url_with_commit_ref = (
            origin_root_dir + "@4091c7d010ca99d0f2dd416d4b70b758ae432187"
        )

        git_wrapper = gclient_scm.GitWrapper(
            url_with_commit_ref, self.root_dir, self.relpath
        )
    ...
    def testUpdateCloneOnBranch(self):
        ...
        git_wrapper = gclient_scm.GitWrapper(
            url_with_branch_ref, self.root_dir, self.relpath
        )
...
class CipdWrapperTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self._cipd_root = gclient_scm.CipdRoot(
            self._cipd_root_dir, self._cipd_instance_url
        )
        self._cipd_packages = [
...
class GcsWrapperTestCase(unittest.TestCase):
    ...
    def createScm(self):
        return gclient_scm.GcsWrapper(
            url=self.url, root_dir=self.workdir, relpath="fake_relpath"
        )
...
class GerritChangesFakeRepo(fake_repos.FakeReposBase):
    def populateGit(self):
        ...
        self._commit_git(
            "repo_1", {"commit 5": "touched", "change": "1234"}, base=3
        )
        ...
        self._commit_git(
            "repo_1", {"commit 10": "touched", "change": "1236"}, base=8
        )
...
class GerritChangesTest(fake_repos.FakeReposTestBase):
    ...
    def assertCommits(self, commits):
        ...
        for i in all_commits - set(commits):
            name = os.path.join(self.root_dir, "commit " + str(i))
            self.assertFalse(
                os.path.exists(name), "Unexpected commit: %s" % name
            )
    ...
    def testCanCloneGerritChange(self):
        ...
        git_wrapper.update(self.options, None, file_list)
        self.assertEqual(
            self.githash("repo_1", 6), self.gitrevparse(self.root_dir)
        )
    ...
    def testCanSyncToGerritChange(self):
        ...
        git_wrapper.update(self.options, None, file_list)
        self.assertEqual(
            self.githash("repo_1", 1), self.gitrevparse(self.root_dir)
        )
    ...
    def testCanCloneGerritChangeMirror(self):
        with mock.patch(
            "git_cache.Mirror.contains_revision",
    ...
    def setUpBootstrapMode(self):
        self.setUpMirror()
        gclient_scm.GitWrapper.cache_mode = gclient_scm.CacheMode.BOOTSTRAP
        self.addCleanup(
            setattr,
    ...
    def testBootstrapModeDetachesCheckoutFromMirror(self):
        self.setUpBootstrapMode()
        ...
        # Configured like a checkout made without a cache: both remote URLs
        # on the real remote, no alternates borrowing from the mirror.
        fetch_url = git_wrapper._Capture(["remote", "get-url", "origin"])
        self.assertEqual(fetch_url, self.url)
        push_url = git_wrapper._Capture(
            ["remote", "get-url", "--push", "origin"]
        )
        self.assertEqual(push_url, self.url)
        self.assertFalse(
            os.path.exists(
                os.path.join(
                    self.root_dir, ".git", "objects", "info", "alternates"
                )
            )
        )

        # The checkout must survive deletion of the cache.
        gclient_utils.rmtree(self.mirror)
        git_wrapper._Capture(["fsck", "--no-progress"])

    def testBootstrapModeExistingCheckoutIgnoresMirror(self):
        self.setUpBootstrapMode()

        git_wrapper = gclient_scm.GitWrapper(self.url, self.root_dir, ".")
        git_wrapper.update(self.options, None, [])

        # Only a checkout that does not exist yet may use the mirror.
        self.assertIsNone(git_wrapper._GetMirror(self.url, self.options))

        # A later sync keeps the detached configuration.
        git_wrapper.update(self.options, None, [])
        fetch_url = git_wrapper._Capture(["remote", "get-url", "origin"])
        self.assertEqual(fetch_url, self.url)

    def testBootstrapModeKeepsExistingSharedCheckout(self):
        self.setUpMirror()

        # Sync in the default shared topology: origin on the mirror, objects
        # borrowed via alternates.
        shared = gclient_scm.GitWrapper(self.url, self.root_dir, ".")
        shared.update(self.options, None, [])
        alternates = os.path.join(
            self.root_dir, ".git", "objects", "info", "alternates"
        )
        self.assertTrue(os.path.exists(alternates))
        ...
        # Bootstrap must not detach it, or deleting the cache corrupts it.
        git_wrapper = gclient_scm.GitWrapper(self.url, self.root_dir, ".")
        self.assertIsNotNone(git_wrapper._GetMirror(self.url, self.options))
        ...
        self.assertTrue(os.path.exists(alternates))
        fetch_url = git_wrapper._Capture(["remote", "get-url", "origin"])
        self.assertTrue(fetch_url.startswith(self.mirror))
    ...
    def testAppliesPatchOnTopOfMasterByDefault(self):
        ...
        git_wrapper.update(self.options, None, file_list)
        self.assertEqual(
            self.githash("repo_1", 4), self.gitrevparse(self.root_dir)
        )
    ...
    def testCheckoutOriginFeature(self):
        ...
        git_wrapper.update(self.options, None, file_list)
        self.assertEqual(
            self.githash("repo_1", 9), self.gitrevparse(self.root_dir)
        )

        # Apply the change on top of that.
        git_wrapper.apply_patch_ref(
            self.url,
            "refs/changes/36/1236/1",
            "refs/heads/feature",
            self.options,
            file_list,
        )
    ...
    def testCheckoutOriginFeatureOnOldRevision(self):
        ...
        git_wrapper.update(self.options, None, file_list)
        self.assertEqual(
            self.githash("repo_1", 7), self.gitrevparse(self.root_dir)
        )
    ...
    def testIgnoresAlreadyMergedCommits(self):
        ...
        git_wrapper.update(self.options, None, file_list)
        self.assertEqual(
            self.githash("repo_1", 12), self.gitrevparse(self.root_dir)
        )
    ...
    def testRecoversFromExistingCherryPick(self):
        ...
        git_wrapper._Run(
            ["fetch", "origin", "refs/changes/36/1236/1"], self.options
        )
        with self.assertRaises(subprocess2.CalledProcessError) as cm:
...
class CheckDiffTest(fake_repos.FakeReposTestBase):
    ...
    def testCheckDiff(self):
        ...
        self.assertFalse(
            git_wrapper.check_diff(
                self.githash("repo_1", 2), files=["DEPS", "doesnotmatter"]
            )
        )
...
class Submodules(BaseGitWrapperTestCase):
    ...
    def testGetSubmoduleClean(self):
        ...
        git_wrapper.update(options, None, [])
        self.assertEqual(
            git_wrapper.GetSubmoduleStateFromIndex(),
