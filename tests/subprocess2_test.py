    ...
    def wrapper(test):
        def inner(self):
            for subp in subps:
                ...
                test(
                    self, to_crlf_bytes, TEST_COMMAND + ["--crlf"], False, subp
                )
...
class SmokeTests(unittest.TestCase):
    ...
    def test_print_exception(self):
        ...
        self.assertIn(
            e.exception.stdout.decode("utf-8", "ignore"), exception_str
        )
    ...
    def test_stdin(self, c, cmd, un, subp):
        ...
        res = subprocess2.communicate(
            cmd + ["--read"], stdin=stdin, universal_newlines=un
        )
    ...
    def test_stdin_void(self):
        res = subprocess2.communicate(
            TEST_COMMAND + ["--read"], stdin=subprocess2.DEVNULL
        )
        self._check_res(res, None, None, 0)
