        ...
        for path in self.all_repos:
            with self.subTest(path=path):
                ...
                readme_path = os.path.join(
                    self.root_dir, path, os.path.pardir, "README.chromium"
                )
                ...
                if path == "src/third_party/not_supported/no_revision/src":
                    self.assertIn(
                        "README.chromium contains 0 Revision: lines", stdout
                    )
...
class CommitMessageTest(unittest.TestCase):
    ...
    def testShouldNotShowLogWithPrivateHost(self):
        self.assertFalse(
            roll_dep.should_show_log("https://private.googlesource.com/project")
        )
