...
class GclientTest(trial_dir.TestCase):
    ...
    def testCustomHooks(self):
        extra_hooks = [
            {"name": "append", "pattern": ".", "action": ["supercmd"]}
        ]
    ...
    def testRecurseDepsAndHooks(self):
        ...
        self.assertEqual(
            [h.action for h in self._get_hooks()], [("tata", "titi")]
        )
    ...
    def testTargetOsForHooksInDepsFile(self):
        ...
        self.assertEqual(
            [h.action for h in all_hooks],
            [("python", "do_a"), ("python", "do_b")],
        )
        self.assertEqual(
    ...
    def testRecursedepsCustomdepsOverride(self):
        ...
        write(
            os.path.join("foo", "bar", "DEPS"), 'deps = {\n  "baz": "/baz",\n}'
        )
    ...
    def testDepsFromNotAllowedHostsBad(self):
        ...
        self.assertEqual(
            [dep.dependencies[0]], dep.findDepsFromNotAllowedHosts()
        )
        self._get_processed()
    ...
    def testParseDepsFile_FalseShouldSync_WithCustoms(self):
        ...
        hooks = [
            {"name": "overridehook", "pattern": ".", "action": ["echo", "cow"]},
            {"name": "notouchhook", "pattern": ".", "action": ["echo", "fail"]},
        ]
        pre_deps_hooks = [
            {"name": "runfirst", "pattern": ".", "action": ["echo", "prehook"]}
        ]
        write(
            os.path.join("chicken", "DEPS"),
            "deps = %s\n"
            "hooks = %s\n"
            "pre_deps_hooks = %s"
        ...
        self.assertCountEqual(
            expected_dep_names, [d.name for d in sol.dependencies]
        )
        self.assertCountEqual(
            expected_hook_names, [h.name for h in sol._deps_hooks]
        )
    ...
    submodule.a b/c.url=http://example.com/a%20b/c"""
        ...
        subprocess2_check_output_mock = mock.MagicMock(
            side_effect=[git_config.encode(), ls_files.encode()]
        )
        ...
        with (
            mock.patch("os.path.isfile", os_path_isfile_mock),
            mock.patch(
    ...
    def testSameDirAllowMultipleCipdDeps(self):
        ...
        self.assertEqual(
            "https://example.com/foo_package@foo_version", dep0.url
        )
        self.assertEqual(
            "https://example.com/bar_package@bar_version", dep1.url
        )
    ...
    def testEnforceSkipSyncRevisions_DepsPatchRefs(self):
        ...
        write(
            os.path.join("foo/src", "DEPS"),
            'deps = {\n  "bar": "https://example.com/bar.git@bar_version",\n}',
        )
        write(
            gclient.PREVIOUS_SYNC_COMMITS_FILE, json.dumps({"foo/src": "1234"})
        )
        options, _ = gclient.OptionParser().parse_args([])
    ...
    def testEnforceSkipSyncRevisions_CustomVars(self):
        ...
        write(
            os.path.join("samevars", "DEPS"),
            'deps = {\n  "bar": "https://example.com/bar.git@bar_version",\n}',
        )
        write(
            os.path.join("diffvars", "DEPS"),
            'deps = {\n  "moo": "https://example.com/moo.git@moo_version",\n}',
        )
        write(
            os.path.join("novars", "DEPS"),
            'deps = {\n  "poo": "https://example.com/poo.git@poo_version",\n}',
        )
        previous_custom_vars = {
            "samevars": {"checkout_foo": "true"},
            "diffvars": {"checkout_chicken": "false"},
        }
        write(
            gclient.PREVIOUS_CUSTOM_VARS_FILE, json.dumps(previous_custom_vars)
        )
        ...
        write(
            gclient.PREVIOUS_SYNC_COMMITS_FILE,
