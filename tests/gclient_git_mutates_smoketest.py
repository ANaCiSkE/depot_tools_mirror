...
class GClientSmokeGITMutates(gclient_smoketest_base.GClientSmokeBase):
    ...
    def testRevertAndStatus(self):
        ...
        new_deps = cur_deps.replace(
            "repo_2@%s'" % repo_2_hash, "repo_2@' + Var('r2hash')"
        )
        ...
        self.gclient(
            [
                ...
                "--spec",
                config_template
                % {"git_base": self.git_base, "custom_vars": {}},
            ]
        )
        ...
        out = self.parseGclient(
            ["revert", "--deps", "mac", "--jobs", "1"], ["running", "running"]
        )
    ...
    def testSyncNoHistory(self):
        ...
        self.gclient(
            ["config", "--spec", config_template % {"git_base": self.git_base}]
        )
        ...
        rev_lists = subprocess2.check_output(
            ["git", "rev-list", "HEAD"], cwd=repo2_root
        ).decode("utf-8")
