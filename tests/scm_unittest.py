...
class GitWrapperTestCase(unittest.TestCase):
    ...
    def testListSubmodules(self, mockCapture, *_mock):
        ...
        if sys.platform.startswith("win"):
            self.assertEqual(
                actual_list, ["foo\\path\\script", "foo\\path\\script2"]
            )
        else:
            self.assertEqual(
                actual_list, ["foo/path/script", "foo/path/script2"]
...
class RealGitTest(fake_repos.FakeReposTestBase):
    ...
    def testGetAllFiles(self):
        self.assertEqual(
            ["DEPS", "foo bar", "origin"], scm.GIT.GetAllFiles(self.cwd)
        )

    def testScopedConfig(self):
        scm.GIT.SetConfig(
            self.cwd, "diff.test-key", value="some value", scope="global"
        )
        self.assertEqual(
            scm.GIT.GetConfig(self.cwd, "diff.test-key", None), "some value"
        )
        self.assertEqual(
        ...
        scm.GIT.SetConfig(
            self.cwd, "diff.test-key1", value="some value", scope="local"
        )
        self.assertEqual(
            scm.GIT.GetConfig(self.cwd, "diff.test-key1", None), "some value"
        )
        self.assertEqual(
    ...
    def testGetSetConfig(self):
        ...
        subprocess.run(
            ["git", "config", key, "line 1\nline 2\nline 3"], cwd=self.cwd
        )
        self.assertEqual(
            "line 1\nline 2\nline 3", scm.GIT.GetConfig(self.cwd, key)
        )
        self.assertEqual(
            "line 1\nline 2\nline 3",
        ...
        with self.assertRaises(scm.GitConfigUnsetMissingValue):
            scm.GIT.SetConfig(
                self.cwd, key, None, modify_all=True, missing_ok=False
            )
        with self.assertRaises(scm.GitConfigUnsetMissingValue):
            scm.GIT.SetConfig(
                self.cwd,
    ...
    def testGetSetConfigList(self):
        ...
        self.assertListEqual(
            ["foo", "bar"], scm.GIT.GetConfigList(self.cwd, key)
        )
    ...
    def testFetchUpstreamTuple_GuessOriginMaster(self, _mockGetRemoteBranches):
        self.assertEqual(
            ("origin", "refs/heads/main"), scm.GIT.FetchUpstreamTuple(self.cwd)
        )

    @mock.patch(
        "scm.GIT.GetRemoteBranches",
    ...
    def testFetchUpstreamTuple_RietveldUpstreamConfig(self):
        scm.GIT.SetConfig(
            self.cwd, "rietveld.upstream-branch", "rietveld-upstream"
        )
        scm.GIT.SetConfig(
            self.cwd, "rietveld.upstream-remote", "rietveld-remote"
        )
        self.assertEqual(
...
class DiffTestCase(unittest.TestCase):
    ...
    def testGetAllFiles_ReturnsAllFilesIfNoSubmodules(self, mockListSubmodules):
        ...
        if sys.platform.startswith("win"):
            ...
            self.assertCountEqual(
                files, ["foo/file.txt", "foo/dir/file.txt", "baz_repo/file.txt"]
            )
...
class GitConfigStateTestTest(unittest.TestCase):
    ...
    def _make(
        *,
        ...
        m = scm.GitConfigStateTest(
            threading.Lock(), global_state, system_state=system_state
        )
    ...
    def test_construction_empty(self):
        ...
        self.assertDictEqual(
            m.load_config(),
            {
                "global": {"section.key": ["override"]},
                "default": {"section.key": ["override"]},
            },
        )
    ...
    def test_construction_global(self):
        ...
        self.assertDictEqual(
            self.defaultdict_to_dict(gs), {"section.key": ["global"]}
        )
        self.assertDictEqual(
            self.defaultdict_to_dict(m.load_config()),
            {
                "global": {"section.key": ["global"]},
                "default": {"section.key": ["global"]},
            },
        )
    ...
    @mock.patch("scm.GIT.Capture")
    def test_load_config_git_version_2_old(
        self, mock_capture, mock_get_git_version
    ):
        ...
        mock_capture.assert_called_once_with(
            ["config", "--list", "-z"], cwd=config_state.root, strip_out=False
        )

    @mock.patch("git_common.get_git_version")
    @mock.patch("scm.GIT.Capture")
    def test_load_config_git_version_new(
        self, mock_capture, mock_get_git_version
    ):
    ...
    def test_construction_system(self):
        ...
        self.assertDictEqual(
            self.defaultdict_to_dict(m.load_config()),
            {
                "default": {"section.key": ["system", "global"]},
                "global": {"section.key": ["global"]},
                "system": {"section.key": ["system"]},
            },
        )
        ...
        self.assertDictEqual(
            self.defaultdict_to_dict(m.load_config()),
            {
                "global": {"section.key": ["override"]},
                "system": {"section.key": ["system"]},
                "default": {"section.key": ["system", "override"]},
            },
        )
    ...
    def test_set_config_system(self):
        ...
        with self.assertRaises(scm.GitConfigUneditableScope):
            m.set_config(
                "section.key", "new_global", append=False, scope="system"
            )
    ...
    def test_set_config_unknown(self):
        ...
        with self.assertRaises(scm.GitConfigUnknownScope):
            m.set_config(
                "section.key", "new_global", append=False, scope="meepmorp"
            )
    ...
    def test_set_config_global_append_empty(self):
        ...
        self.assertDictEqual(
            m.load_config(),
            {
                "default": {"section.key": ["new_global"]},
                "global": {"section.key": ["new_global"]},
            },
        )
    ...
    def test_set_config_global(self):
        ...
        self.assertDictEqual(
            self.defaultdict_to_dict(m.load_config()),
            {
                "global": {"section.key": ["new_global", "new_global2"]},
                "default": {"section.key": ["new_global", "new_global2"]},
            },
        )

        self.assertDictEqual(
            self.defaultdict_to_dict(gs),
    ...
    def test_set_config_multi_global(self):
        ...
        m.set_config_multi(
            "section.key", "new_global", value_pattern=None, scope="global"
        )
        self.assertDictEqual(
            self.defaultdict_to_dict(m.load_config()),
            {
        ...
        m.set_config_multi(
            "othersection.key", "newval", value_pattern=None, scope="global"
        )
        self.assertDictEqual(
            m.load_config(),
            {
    ...
    def test_set_config_multi_global_pattern(self):
        ...
        m.set_config_multi(
            "section.key", "new_global", value_pattern="2", scope="global"
        )
        self.assertDictEqual(
            m.load_config(),
            {
                "global": {"section.key": ["1", "1", "new_global", "3"]},
                "default": {"section.key": ["1", "1", "new_global", "3"]},
            },
        )

        m.set_config_multi(
            "section.key", "additional", value_pattern="narp", scope="global"
        )
        self.assertDictEqual(
            m.load_config(),
            {
    ...
    def test_unset_config_global_extra(self):
        ...
        self.assertDictEqual(
            m.load_config(),
            {
                "global": {"extra": ["another"]},
                "default": {"extra": ["another"]},
            },
        )
    ...
    def test_unset_config_multi_global(self):
        ...
        m.unset_config_multi(
            "section.key", value_pattern=None, scope="global", missing_ok=False
        )
        self.assertDictEqual(m.load_config(), {})
    ...
    def test_unset_config_multi_global_pattern(self):
        ...
        m.unset_config_multi(
            "section.key", value_pattern="2", scope="global", missing_ok=False
        )
        self.assertDictEqual(
            m.load_config(),
            {
...
class CanonicalizeGitConfigKeyTest(unittest.TestCase):
    ...
    def test_many(self):
        self.assertEqual(
            self.ck("URL.https://SoMeThInG.example.com.INSTEADOF"),
...
class CachedGitConfigStateTest(unittest.TestCase):
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "value_pattern.*modify_all.*invalid"
        ):
            gcs.SetConfig(
                "section.variable",
                "value",
        ...
        with self.assertRaisesRegex(
            ValueError, "value_pattern.*append.*invalid"
        ):
            gcs.SetConfig(
                "section.variable",
                "value",
    ...
    def test_get_config(self):
        ...
        self.assertListEqual(
            gcs.GetConfigList("seCTIon.vARIable"),
            [
                "value",
                "value_bleem",
                "true",
