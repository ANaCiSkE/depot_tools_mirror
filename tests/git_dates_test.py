...
class GitDatesTest(GitDatesTestBase):
    def testTimestampOffsetToDatetime(self):
        ...
        offset = "+1100"
        expected_tz = self.git_dates.FixedOffsetTZ(
            datetime.timedelta(hours=11), ""
        )
        expected = datetime.datetime(
            2016, 1, 25, 17, 25, 43, tzinfo=expected_tz
        )
        ...
        expected = datetime.datetime(
            2016, 1, 24, 22, 25, 43, tzinfo=expected_tz
        )
        result = self.git_dates.timestamp_offset_to_datetime(timestamp, offset)
        self.assertEqual(expected, result)
    ...
    def testDatetimeString(self):
        ...
        self.assertEqual(
            "2016-01-25 17:25:43 +1100", self.git_dates.datetime_string(dt)
        )

        tz = self.git_dates.FixedOffsetTZ(datetime.timedelta(hours=-8), "")
        dt = datetime.datetime(2016, 1, 24, 22, 25, 43, tzinfo=tz)
        self.assertEqual(
            "2016-01-24 22:25:43 -0800", self.git_dates.datetime_string(dt)
        )
