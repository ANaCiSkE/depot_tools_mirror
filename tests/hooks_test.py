...
class HooksTest(unittest.TestCase):
    ...
    def populate(self):
        ...
        gitmodules_content = "\n".join(
            (
                '[submodule "dep_a"]\tpath = dep_a',
                "\turl = host://dep_a",
                '[submodule "dep_b"]\tpath = dep_b',
                "\turl = host://dep_b",
            )
        )
    ...
    def testPreCommit_NoGitlinkOrDEPS(self):
        ...
        self.assertEqual(
            expected_diff, git.run("diff", "HEAD^", "HEAD", cwd=self.repo)
        )
    ...
    def testPreCommit_GitlinkWithoutDEPS(self):
        ...
        diff_before_commit = git.run(
            "diff", "--cached", "--name-only", cwd=self.repo
        )
        _, stderr = git.run_with_stderr(
            "commit",
            "-m",
        ...
        diff_after_commit = git.run(
            "diff", "--name-only", "HEAD^", "HEAD", cwd=self.repo
        )
    ...
    def testPreCommit_OnlyGitlinkWithoutDEPS(self):
        ...
        ret = git.run_with_retcode(
            "commit", "-m", "gitlink only", cwd=self.repo, env=self.env
        )
    ...
    def testPreCommit_CommitAll(self):
        ...
        diff_before_commit = git.run("status", cwd=self.repo)
        self.assertIn("foo", diff_before_commit)
        self.assertIn("dep_a", diff_before_commit)
        ret = git.run_with_retcode(
            "commit", "--all", "-m", "commit all", cwd=self.repo, env=self.env
        )
