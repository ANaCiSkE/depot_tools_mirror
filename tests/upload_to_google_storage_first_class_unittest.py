...
class UploadTests(unittest.TestCase):
    ...
    def test_upload_single_file_remote_exists(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                (
                    "check_call",
                    ("ls", "-L", "%s/%s" % (self.base_url, object_name)),
                ),
            ],
        )
    ...
    def test_create_archive(self):
        ...
        with ChangedWorkingDirectory(work_dir):
            ...
            dirs = [dirname]
            self.assertTrue(
                upload_to_google_storage_first_class.validate_archive_dirs(dirs)
            )
    ...
    @unittest.skipIf(
        sys.platform == "win32", "os.symlink does not exist on win"
    )
    ...
    def test_get_targets_no_args(self):
        try:
            upload_to_google_storage_first_class.get_targets(
                [], self.parser, False
            )
            self.fail()
