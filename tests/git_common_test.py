...
class ProgressPrinterTest(GitCommonTestBase):
    ...
    def testBasic(self):
        ...
        pp = self.gc.ProgressPrinter(
            fmt, enabled=True, fout=stream, period=0.01
        )
        with pp as inc:
            for _ in range(10):
                time.sleep(0.02)
...
class GitReadOnlyFunctionsTest(
    git_test_utils.GitRepoReadOnlyTestBase, GitCommonTestBase
):
    REPO_SCHEMA = """
  A B C D
    B E D
  """
    ...
    COMMIT_C = {
        "some/files/file2": {"mode": 0o755, "data": b"file2 - vanilla\n"},
    }
    ...
    def testHashes(self):
        ...
        self.assertEqual(
            self.repo.run(self.gc.hash_one, "branch_D"), self.repo["D"]
        )
        self.assertTrue(
            self.repo["D"].startswith(
    ...
    def testStreamWithRetcode(self):
        ...
        def testfn():
            with self.gc.run_stream_with_retcode(
                "log", "--format=%H"
            ) as stdout:
    ...
    def testBranches(self):
        ...
        self.assertEqual(
            self.repo.run(set, self.gc.branches()),
            {"main", "branch_D", "root_A"},
        )

    def testDiff(self):
        # Get the names of the blobs being compared (to avoid hard-coding).
        c_blob_short = self.repo.git(
            "rev-parse", "--short", "tag_C:some/files/file2"
        ).stdout.strip()
        d_blob_short = self.repo.git(
            "rev-parse", "--short", "tag_D:some/files/file2"
        ).stdout.strip()
        expected_output = [
    ...
    def testDormant(self):
        ...
        self.gc.scm.GIT.SetConfig(
            self.repo.repo_path, "branch.main.dormant", "true"
        )
    ...
    def testBlame(self):
        def get_porcelain_for_commit(commit_name, lines):
            ...
            info = self.repo.show_commit(
                commit_name, format_string=format_string
            )
            return info.split("\n")
        ...
        self.assertEqual(
            expected_output,
            self.repo.run(
                self.gc.blame, "some/files/file2", "tag_D", porcelain=True
            ).split("\n"),
        )
    ...
    def testRepoRoot(self):
        ...
        self.assertEqual(
            self.repo.repo_path, self.repo.run(cd_and_repo_root, "some/files")
        )
...
class GitMutableFunctionsTest(
    ...
    def testInternF(self):
        ...
        self.assertEqual(
            data, self.repo.git("cat-file", "blob", data_hash).stdout
        )
    ...
    def testConfig(self):
        ...
        self.assertEqual(
            self.repo.run(self.gc.get_config_list, "sad.derpies"), []
        )
...
class GitMutableStructuredTest(
  ...
  """
    ...
    def testTooManyBranches(self):
        ...
        self.assertIn("too many branches (39/20)", rslt)
        self.gc.scm.GIT.SetConfig(
            self.repo.repo_path, "depot-tools.branch-limit", "100"
        )
    ...
    def testMergeBase(self):
        ...
        self.assertEqual(
            self.repo["B"],
            self.repo.run(
                self.gc.get_or_create_merge_base, "branch_K", "branch_G"
            ),
        )
        ...
        self.assertEqual(
            self.repo["J"],
            self.repo.run(
                self.gc.get_or_create_merge_base, "branch_L", "branch_K"
            ),
        )
        ...
        self.repo.run(
            self.gc.manual_merge_base, "branch_K", "deadbeef", "branch_G"
        )
        ...
        self.repo.run(
            self.gc.manual_merge_base, "branch_K", self.repo["I"], "branch_G"
        )
        ...
        self.assertEqual(
            {"branch_K": self.repo["I"], "branch_L": self.repo["J"]},
        ...
        self.repo.run(
            self.gc.manual_merge_base, "branch_K", self.repo["A"], "branch_G"
        )
    ...
    def testGetHashes(self):
        ...
        for branch, branch_hash in hashes.items():
            self.assertEqual(
                self.repo.run(self.gc.hash_one, branch), branch_hash
            )
  ...
  """
    ...
    def testAll(self):
        def inner():
            ...
            self.assertIsNotNone(
                self.gc.thaw()
            )  # One thaw should thaw everything
...
class GitMakeWorkdir(git_test_utils.GitRepoReadOnlyTestBase, GitCommonTestBase):
  ...
  """
    ...
    def testMakeWorkdir(self):
        ...
        self.gc.make_workdir(
            os.path.join(self.repo.repo_path, ".git"),
...
class GitTestUtilsTest(git_test_utils.GitRepoReadOnlyTestBase):
  ...
  """
    ...
    COMMIT_C = {
        ...
        GitRepo.AUTHOR_DATE: datetime.datetime(
            1980, 9, 8, 7, 6, 5, tzinfo=git_test_utils.UTC
        ),
        GitRepo.COMMITTER_NAME: "Custom Committer",
        GitRepo.COMMITTER_EMAIL: "committer@example.com",
        GitRepo.COMMITTER_DATE: datetime.datetime(
            1990, 4, 5, 6, 7, 8, tzinfo=git_test_utils.UTC
        ),
    ...
    def testAutomaticCommitDates(self):
        ...
        self.assertEqual(
            "Author McAuthorly 1970-01-01 00:00:00 +0000",
            self.repo.show_commit("A", format_string="%an %ai"),
        )
        self.assertEqual(
            "Charles Committish 1970-01-02 00:00:00 +0000",
            self.repo.show_commit("A", format_string="%cn %ci"),
        )
        self.assertEqual(
            "Author McAuthorly 1970-01-03 00:00:00 +0000",
            self.repo.show_commit("B", format_string="%an %ai"),
        )
        self.assertEqual(
            "Charles Committish 1970-01-04 00:00:00 +0000",
            self.repo.show_commit("B", format_string="%cn %ci"),
        )

    def testCustomCommitData(self):
        self.assertEqual(
            "Custom Author author@example.com 1980-09-08 07:06:05 +0000",
            self.repo.show_commit("C", format_string="%an %ae %ai"),
        )
        self.assertEqual(
            "Custom Committer committer@example.com 1990-04-05 06:07:08 +0000",
...
class WarnSubmoduleTest(unittest.TestCase):
    ...
    def testWarnFSMonitorOldVersion(self):
        ...
        self.warn_submodule()
        self.assertTrue(
            "WARNING: You have fsmonitor enabled." in sys.stdout.getvalue()
        )
    ...
    def testWarnFSMonitorGoogVersion(self):
        ...
        mock.patch("sys.platform", "darwin").start()
        mock.patch(
            "git_common.run", lambda _: "git version 2.42.0.515.A-goog"
        ).start()
        self.warn_submodule()
        self.assertFalse(
            "WARNING: You have fsmonitor enabled." in sys.stdout.getvalue()
...
class ExtractGitPathFromGitBatTest(GitCommonTestBase):
    def test_unexpected_format(self):
        git_bat = os.path.join(
            DEPOT_TOOLS_ROOT,
            "tests",
            "git_common_test.inputs",
            "testGitBatUnexpectedFormat",
            "git.bat",
        )
