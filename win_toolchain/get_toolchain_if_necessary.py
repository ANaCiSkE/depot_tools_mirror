...
def GetFileList(root):
    ...
    for base, _, files in os.walk(root):
        ...
        for p in paths:
            if any(
                ignored_dir in p.lower() for ignored_dir in ignored_directories
            ):
                continue
            file_list.append(p)
...
def CalculateHash(root, expected_hash):
    ...
    if matches:
        ...
        if len(missing_files):
            print(
                "%d files missing from the %s version of the toolchain:"
        ...
        if len(extra_files):
            print(
                "%d extra files in the %s version of the toolchain:"
    ...
    print(
        "Calculating hash of toolchain in %s. Please wait..." % full_root_path
    )
    sys.stdout.flush()
    digest = hashlib.sha1()
...
def DelayBeforeRemoving(target_dir):
    ...
    if os.path.isdir(target_dir) and not bool(
        int(os.environ.get("CHROME_HEADLESS", "0"))
    ):
...
def DownloadUsingHttp(filename):
    ...
    try:
        with closing(urlopen(src_url)) as fsrc, open(target_path, "wb") as fdst:
...
def DownloadUsingGsutil(filename):
    ...
    code = gsutil.call(
        "cp", "gs://chrome-wintoolchain/" + filename, target_path
    )
    if code != 0:
...
def RemoveUnusedToolchains(root):
    ...
    for d in dirs_to_remove:
        print(
            "Removing %s as it doesn't correspond to any known toolchain."
            % os.path.join(root, d)
        )
...
def SetupJunction(target_dir):
    ...
    subprocess.run(
        ["mklink", "/J", junction_dir, target_dir], shell=True, check=True
    )
    return junction_dir
...
def main():
    ...
    if args.output_json:
        if not os.path.exists(args.output_json) or not filecmp.cmp(
            data_path, args.output_json
        ):
