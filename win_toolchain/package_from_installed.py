...
def GetVSPath():
    ...
    if vs_installs_count == 0:
        raise Exception(
            "VS %s path not found in vswhere output" % (_vs_version)
        )
    if vs_installs_count > 1:
        if not _allow_multiple_vs_installs:
            ...
            print(
                "Multiple VS installs were detected. This is unsupported. "
                "Proceeding anyway"
            )
...
def ExpandWildcards(root, sub_dir):
    ...
    if len(matches) != 1:
        raise Exception(
            "%s had %d matches - should be one" % (path, len(matches))
        )
    return matches[0]
...
def BuildFileList(override_dir, include_arm, vs_path):
            ...
            for f in files:
                ...
                if isinstance(path, tuple):
                    ...
                    result.append(
                        (
                            final_from,
                            os.path.normpath(os.path.join(path[1], dest)),
                        )
                    )
                else:
    ...
    for root, _, files in os.walk(sdk_path):
        for f in files:
            ...
            if (
                tail.startswith("Include\\")
                or tail.startswith("Lib\\")
                or tail.startswith("Source\\")
                or tail.startswith("bin\\")
            ):
    ...
    for system_crt_file in system_crt_files:
        for cpu_pair in cpu_pairs:
            ...
            src_path = os.path.join(
                sdk_path, "bin", _win_version, target_cpu, "ucrt"
            )
            result.append(
                (
                    os.path.join(src_path, system_crt_file),
    ...
    def is_skippable(f):
        return (
            ...
            or "samples\\" in f.lower()
            or f.lower().endswith((".msi", ".msm"))
        )
...
def AddEnvSetup(files, include_arm):
    ...
    files.append(
        (
            os.path.join(
                tempdir, "Windows Kits", "10", "bin", "SetEnv.x86.json"
            ),
    ...
    files.append(
        (
            os.path.join(
                tempdir, "Windows Kits", "10", "bin", "SetEnv.x64.json"
            ),
    ...
    if include_arm:
        files.append(
            (
                os.path.join(
                    tempdir, "Windows Kits", "10", "bin", "SetEnv.arm64.json"
                ),
...
def RenameToSha1(output):
    ...
    with zipfile.ZipFile(
        os.path.join(old_dir, output), "r", zipfile.ZIP_DEFLATED, True
    ) as zf:
...
def main():
    ...
    if options.repackage_dir:
        ...
        if len(args) != 1 or args[0] != SUPPORTED_VS_VERSION:
            print(
                "This script only supports Visual Studio", SUPPORTED_VS_VERSION
            )
            parser.print_help()
            return 1
        ...
        if options.override_dir:
            if (
                not os.path.exists(os.path.join(options.override_dir, "bin"))
                or not os.path.exists(
                    os.path.join(options.override_dir, "include")
                )
                or not os.path.exists(os.path.join(options.override_dir, "lib"))
            ):
                print(
        ...
        print(
            "Building file list for VS %s Windows %s..."
    ...
    if options.dump_file_list:
        ...
        with open(
            options.dump_file_list, "wt", newline="", encoding="utf-8"
        ) as f:
    ...
    with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED, True) as zf:
        for disk_name, archive_name in files:
            sys.stdout.write(
                "\r%d/%d ...%s" % (count, len(files), disk_name[-40:])
            )
            sys.stdout.flush()
            count += 1
