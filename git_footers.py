...
def main(args):
    if gclient_utils.IsEnvCog():
        print(
            "footers command is not supported in non-git environment",
            file=sys.stderr,
        )
        return 1
    ...
    parser.add_argument(
        "ref",
        nargs="?",
        help="Git ref to retrieve footers from. Omit to parse stdin.",
    )
    ...
    g.add_argument(
        "--json", help="filename to dump JSON serialized footers to."
    )
