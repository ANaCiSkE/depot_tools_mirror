...
_THIRD_PARTY_ROOT = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                 'third_party')
...
def import_module(module_path):
    ...
    for part in parts:
        cur_name = (f'{cur_name}.{part}'
                    if parent else f'_depot_tools_third_party_{part}')
        ...
        if not os.path.exists(target_path):
            raise ImportError(f'Cannot find third_party module {module_path} '
...
def get(module_path):
    ...
    for part in module_path.split('.'):
        cur_name = (f'{cur_name}_{part}' if cur_name
                    == '_depot_tools_third_party' else f'{cur_name}.{part}')
