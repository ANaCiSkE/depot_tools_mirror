...
def get_quarter_of(date):
    begin = datetime_to_midnight(date) - relativedelta(
        months=(date.month - 1) % 3, days=(date.day - 1)
    )
...
def get_year_of(date):
    begin = datetime_to_midnight(date) - relativedelta(
        months=(date.month - 1), days=(date.day - 1)
    )
...
class MyActivity(object):
    ...
    def process_gerrit_issue_replies(replies):
        ret = []
        replies = filter(
            lambda r: "author" in r and "email" in r["author"], replies
        )
    ...
    def filter_modified_monorail_issue(self, issue):
        ...
        url = (
            "https://monorail-prod.appspot.com/_ah/api/monorail/v1/projects"
            "/%s/issues/%s/comments?maxResults=10000"
        ) % (project, issue_id)
    ...
    def monorail_issue_search(self, project):
        ...
        issues = self.monorail_query_issues(
            project,
            {
                ...
                "q": user_str,
                "publishedMax": "%d"
                % (self.modified_before - epoch).total_seconds(),
                "updatedMin": "%d"
                % (self.modified_after - epoch).total_seconds(),
            },
        )
        ...
        if self.options.completed_issues:
            return [
                ...
                for issue in issues
                if (
                    self.match(issue["owner"])
                    and issue["status"].lower() in ("verified", "fixed")
                )
            ]
    ...
    def monorail_get_issues(self, project, issue_ids):
        return self.monorail_query_issues(
            project, {"maxResults": 10000, "q": "id:%s" % ",".join(issue_ids)}
        )
    ...
    def print_generic(
        ...
        output_format = (
            specific_fmt if specific_fmt is not None else default_fmt
        )
        values = {
            "title": title,
            "url": url,
    ...
    def filter_issue(self, issue, should_filter_by_user=True):
        ...
        if maybe_filter_username(issue["author"]) and self.filter_modified(
            issue["created"]
        ):
            return True
        ...
        for reply in issue["replies"]:
            if self.filter_modified(reply["created"]):
                if not should_filter_by_user:
                    break
                if (
                    username(reply["author"]) == self.user
    ...
    def get_reviews(self):
        ...
        with contextlib.closing(ThreadPool(num_instances)) as pool:
            gerrit_reviews = pool.map_async(
                lambda instance: self.gerrit_search(
    ...
    def dump_json(self, ignore_keys=None):
        ...
        def format_for_json_dump(in_array):
            ...
            for item in in_array:
                ...
                if not url:
                    raise Exception(
                        "Dumped item %s does not specify url" % item
                    )
                output[url] = dict(
...
def main():
    ...
    parser.add_option(
        ...
        % (
            quarter_begin.strftime("%Y-%m-%d"),
            quarter_end.strftime("%Y-%m-%d"),
        ),
    )
    parser.add_option(
        "-Y", "--this_year", action="store_true", help="Use this year's dates"
    )
    parser.add_option(
        "-w",
        "--week_of",
    ...
    parser.add_option(
        "-d", "--deltas", action="store_true", help="Fetch deltas for changes."
    )
    parser.add_option(
    ...
    activity_types_group.add_option(
        "-c", "--changes", action="store_true", help="Show changes."
    )
    activity_types_group.add_option(
        "-i", "--issues", action="store_true", help="Show issues."
    )
    activity_types_group.add_option(
        "-r", "--reviews", action="store_true", help="Show reviews."
    )
    activity_types_group.add_option(
        "--changes-by-issue",
        action="store_true",
    ...
    if not options.begin:
        if options.last_quarter:
            ...
            begin, end = get_week_of(
                datetime.today() - timedelta(days=1 + 7 * options.last_week)
            )
        else:
