...
def GetNameForCommit(sha1):
    name = re.sub(
        r"~.*$", "", git.run("name-rev", "--tags", "--name-only", sha1)
    )
    if name == "undefined":
        name = (
            git.run(
...
def GetMergesForCommit(sha1):
    return [
        ...
        for c in git.run(
            ...
            "--no-abbrev",
            "--grep",
            "cherry picked from commit %s" % sha1,
        ).splitlines()
    ]
...
def main(args):
    if gclient_utils.IsEnvCog():
        print(
            "find-releases command is not supported in non-git environment.",
            file=sys.stderr,
        )
        return 1
    ...
    for arg in args:
        ...
        for merge in merges:
            print(
                "  merged to " + GetNameForCommit(merge) + " (as " + merge + ")"
            )
        if not merges:
