...
def _AtomicFileWriter(path, mode="w"):
    ...
    fd, temp_path = tempfile.mkstemp(
        dir=os.path.dirname(path), text="b" not in mode
    )
    try:
...
class GerritCache(object):
    ...
    def _get_cache_path(self):
        ...
        try:
            path = scm.GIT.GetConfig(
                self.root_dir, "depot-tools.gerrit-cache-path"
            )
            if path:
                return path
        except Exception:
        ...
        try:
            if self.root_dir:
                ...
                sanitized_path = re.sub(
                    r"[^a-zA-Z0-9]", "_", os.path.abspath(self.root_dir)
                )
                filename = "depot_tools_gerrit_cache_%s.json" % sanitized_path
                path = os.path.join(tempfile.gettempdir(), filename)
            else:
                fd, path = tempfile.mkstemp(
                    prefix="depot_tools_gerrit_cache_", suffix=".json"
                )
                os.close(fd)
            ...
            try:
                scm.GIT.SetConfig(
                    self.root_dir, "depot-tools.gerrit-cache-path", path
                )
            except Exception:
