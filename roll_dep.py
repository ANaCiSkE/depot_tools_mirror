...
_DEPENDENCY_DIVIDER_PATTERN = re.compile(
    r"^-{20} DEPENDENCY DIVIDER -{20}$", re.MULTILINE
)
_REVISION_LINE_PATTERN = re.compile(
    r"^Revision: ([a-f0-9]+|DEPS)$", re.MULTILINE
)
...
def is_pristine(root):
    ...
    return (
        not check_output(diff_cmd, cwd=root).strip()
...
def generate_commit_message(
    full_dir, dependency, head, roll_to, upstream_url, show_log, log_limit
):
    ...
    header = (
        f"Roll {dependency}/ {commit_range_for_header} "
...
def get_submodule_rev(submodule):
    ...
    rev_output = check_output(
        ["git", "submodule", "status", submodule], cwd=gclient(["root"])
    ).strip()
...
def calculate_roll(full_dir, dependency, roll_to):
    ...
    if roll_to == "origin/HEAD":
        check_output(
            ["git", "remote", "set-head", "origin", "-a"], cwd=full_dir
        )
...
def finalize(args, commit_msg, current_dir, rolls):
    ...
    with tempfile.NamedTemporaryFile("w+", newline="", delete=False) as f:
        ...
        f.write(commit_msg)
    check_call(
        ["git", "commit", "--quiet", "--file", commit_filename], cwd=current_dir
    )
...
def update_readme_chromium(dependency, roll_to, current_dir):
    ...
    readme_path = os.path.normpath(
        os.path.join(
            gclient_root, dependency, os.path.pardir, "README.chromium"
        )
    )
    ...
    if _DEPENDENCY_DIVIDER_PATTERN.match(content):
        print(
            'README.chromium contains "- DEPENDENCY DIVIDER -"\n'
    ...
    if revision_count != 1:
        print(
            f"README.chromium contains {revision_count} Revision: lines, skipping update.\n"
            "Files with multiple dependencies are not supported"
        )
        return

    # Update the revision line.
    new_content = _REVISION_LINE_PATTERN.sub(f"Revision: {roll_to}", content)

    if new_content == content:
        print(
            f"README.chromium already has revision {roll_to}, \ncontent:{new_content}"
        )
        return
...
def main():
    if gclient_utils.IsEnvCog():
        print(
            '"roll-dep" is not supported in non-git environment',
            file=sys.stderr,
        )
        return 1
    ...
    parser.add_argument(
        "-b", "--bug", help="Associate a bug number to the roll"
    )
    # It is important that --no-log continues to work, as it is used by
    # internal -> external rollers. Please do not remove or break it.
    parser.add_argument(
    ...
    try:
        ...
        for dependency in dependencies:
            ...
            if not os.path.isdir(full_dir):
                ...
                if not os.path.isdir(full_dir):
                    raise Error(
                        f"Directory not found: {dependency} ({full_dir})"
                    )
        ...
        for dependency, (head, roll_to, full_dir) in sorted(rolls.items()):
            upstream_url = check_output(
                ["git", "config", "remote.origin.url"], cwd=full_dir
            ).strip()
    ...
    if not reviewers:
        print(
            "You forgot to pass -r, make sure to insert a R=foo@example.com "
            "line"
        )
