...
def _replace_ast(
    destination: str, dest_ast: ast.Module, source: str, source_ast: ast.Module
) -> str:
    ...
    if source_ast.lineno == source_ast.end_lineno:
        ...
        result += source_lines[source_ast.lineno - 1][
            source_ast.col_offset : source_ast.end_col_offset
        ]
    else:
        ...
        result += (
            source_lines[source_ast.lineno - 1][source_ast.col_offset :] + "\n"
        )
        ...
        result += source_lines[source_ast.end_lineno - 1][
            : source_ast.end_col_offset
        ]
...
def copy_packages(
    ...
    for i in range(len(source_packages)):
        ...
        if destination_package not in dest_deps:
            raise Exception(
                "Package %s not found in destination" % destination_package
            )
...
def main():
    ...
    parser.add_argument(
        "--source-deps",
        required=True,
        help="Source DEPS file where content will be copied from",
    )
    parser.add_argument(
        "--source-package",
        action="append",
    ...
    parser.add_argument(
        "--destination-deps",
        required=True,
        help="Destination DEPS file, where content will be saved",
    )
    parser.add_argument(
        "--destination-package",
        action="append",
    ...
    if len(args.destination_package) != len(args.source_package):
        parser.error(
            "Source and destination packages must be of the same "
            "length, aborting..."
        )
    ...
    new_content = copy_packages(
        source_content,
        destination_content,
        args.source_package,
        args.destination_package,
    )
    ...
    print("Run:")
    print(
        "  Destination DEPS file updated. You still need to create and "
        "upload a change."
    )
    return 0
