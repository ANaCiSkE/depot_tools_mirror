...
class GclientCheckout(Checkout):
    def run_gclient(self, *cmd, **kwargs):
        if not spawn.find_executable("gclient"):
            cmd_prefix = (
                sys.executable,
                os.path.join(SCRIPT_PATH, "gclient.py"),
            )
        else:
            cmd_prefix = ("gclient",)
    ...
    def exists(self):
        try:
            ...
            return os.path.exists(
                os.path.join(gclient_root, ".gclient")
            ) or os.path.exists(os.path.join(os.getcwd(), self.root, ".git"))
...
def handle_args(argv):
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "On macOS, prevent idle sleep during the operation. Enabled by"
    ...
    parser.add_argument(
        "config", type=str, help="Project to fetch, e.g. chromium."
    )
    parser.add_argument(
        "props", metavar="props", type=str, nargs=argparse.REMAINDER, default=[]
    )
