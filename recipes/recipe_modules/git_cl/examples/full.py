...
def RunSteps(api):
  ...
  result = api.git_cl.get_description(
    patch_url="https://code.review/123", suffix="build"
  )
  api.git_cl.set_description("bammmm", patch_url="https://code.review/123")
...
def GenTests(api):
  yield (
    ...
    + api.override_step_data(
      "git_cl description (2)", stdout=api.raw_io.output("new description woo")
    )
  )
