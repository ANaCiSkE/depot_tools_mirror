...
def RunSteps(api):
  ...
  submodule_update_recursive = api.properties.get(
    "submodule_update_recursive", True
  )

  # You can use api.git.checkout to perform all the steps of a safe checkout.
  revision = (
    api.buildbucket.gitiles_commit.ref or api.buildbucket.gitiles_commit.id
  )
  ...
  assert retVal == "deadbeef", "expected retVal to be %r but was %r" % (
    "deadbeef",
    retVal,
  )
  ...
  if api.properties.get("cat_file", None):
    step_result = api.git.cat_file_at_commit(
      api.properties["cat_file"], revision, stdout=api.raw_io.output()
    )
    if "TestOutput" in step_result.stdout.decode("utf-8"):
      pass  # Success!

  # Bundle the repository.
  api.git.bundle_create(api.path.start_dir / "all.bundle")
...
def GenTests(api):
  ...
  yield (
    api.test("can_fail_build", status="INFRA_FAILURE")
  ...
  yield (api.test("set_got_revision") + api.properties(set_got_revision=True))

  yield (
    api.test("rebase_failed", status="INFRA_FAILURE")
  ...
  yield (
    api.test("count-objects_delta") + api.properties(display_fetch_size=True)
  )

  yield (
    api.test("count-objects_failed") + api.step_data("count-objects", retcode=1)
  )
  ...
  yield (
    api.test(
      "count-objects_with_bad_output_fails_build", status="INFRA_FAILURE"
    )
    + api.step_data(
      "count-objects",
      stdout=api.raw_io.output(api.git.count_objects_output("xxx")),
  ...
  yield (
    ...
    + api.step_data(
      "git cat-file abcdef12345:TestFile",
  ...
  yield (api.test("git-cache-checkout") + api.properties(use_git_cache=True))

  yield (
    api.test("new_branch_failed", status="INFRA_FAILURE")
  ...
  yield (
    api.test("git-fetch-without-auto-gc") + api.properties(no_auto_gc=True)
  )

  yield (
    api.test("git-fetch-with-maintenance")
