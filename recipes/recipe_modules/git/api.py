...
class GitApi(recipe_api.RecipeApi):
  ...
  def __call__(self, *args, **kwargs):
    ...
    with self.m.context(cwd=(self.m.context.cwd or self.m.path.checkout_dir)):
      return self.m.step(
        name, git_cmd + list(args), infra_step=infra_step, **kwargs
      )
  ...
  def cat_file_at_commit(
    self, file_path, commit_hash, remote_name=None, **kwargs
  ):
    ...
    return self(
      "cat-file", "blob", "%s:%s" % (commit_hash, file_path), **kwargs
    )

  def count_objects(
    self, previous_result=None, raise_on_failure=False, **kwargs
  ):
    ...
    try:
      ...
      if previous_result:
        ...
        step_result.presentation.logs["delta"] = (
          ["before:"]
          + results_to_text(previous_result)
          + ["", "after:"]
          + results_to_text(result)
          + ["", "delta:"]
          + results_to_text(delta)
        )
        ...
        size_delta = (
          ...
          - previous_result["size-pack"]
        )
        # size_delta is in KiB.
        step_result.presentation.step_text = "size delta: %+.2f MiB" % (
          size_delta / 1024.0
        )
  ...
  def checkout(
    ...
    path = self.m.path.pathsep.join([str(self.repo_resource()), "%(PATH)s"])
    ...
    with self.m.context(cwd=dir_path):
      ...
      clean_args = list(
        itertools.chain(*[("-e", path) for path in keep_paths or []])
      )
    ...
    with self.m.context(cwd=dir_path):
      try:
        self(
          "rebase",
          "%s/main" % remote_name,
          name="%s rebase" % name_prefix,
          **kwargs,
        )
      except self.m.step.StepFailure:
        self(
          "rebase", "--abort", name="%s rebase abort" % name_prefix, **kwargs
        )
        raise
  ...
  def config_get(self, prop_name, **kwargs):
    ...
    result = self(
      "config", "--get", prop_name, stdout=self.m.raw_io.output(), **kwargs
    )
  ...
  def new_branch(
    self, branch, name=None, upstream=None, upstream_current=False, **kwargs
  ):
  ...
  def ls_remote(self, url, ref, name=None, **kwargs):
    ...
    with self.m.context(cwd):
      result = self(
        *cmd, name=name, stdout=self.m.raw_io.output_text(), **kwargs
      )
