  ...
  def get_files_affected_by_patch_with_submodules(
    ...
    **kwargs,
    ...
    Args:
      ...
      * report_files_via_property: name of the output property to report the
        list of the files. If None (default), do not report.
      * step_name_prefix: optional prefix to prepend to step names (e.g.
        '[Experimental] '). Defaults to ''.
    """
    affected_files = []
    ...
    with self.m.context(cwd=cwd):
      step_result = self.m.git(
        ...
        "--cached",
        "--raw",
        name=f"{step_name_prefix}git diff --raw to analyze patch",
    ...
    if submodules_to_process:
      with self.m.context(cwd=cwd):
        with self.m.step.nest(
          f"{step_name_prefix}git diff submodules"
        ) as presentation:
