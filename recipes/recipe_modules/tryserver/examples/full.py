...
def RunSteps(api):
  ...
  if api.properties.get("patch_text"):
    ...
    api.step(
      ...
      [
        "echo",
        str(api.tryserver.get_footer("Foo", api.properties["patch_text"])),
      ],
    )
    return
  ...
  if api.tryserver.gerrit_change:
    ...
    assert api.tryserver.gerrit_change_repo_host == "chromium.googlesource.com"
    assert api.tryserver.gerrit_change_repo_project == "chromium/src"
    ...
    assert (
      api.tryserver.gerrit_change_review_url
...
def GenTests(api):
  ...
  yield (
    api.test("with_wrong_patch")
    + api.platform("win", 32)
    + api.properties(test_patch_root="")
  )

  yield (
    api.test("with_gerrit_patch")
  ...
  yield (
    api.test("with_gerrit_patch_and_target_ref")
    + api.buildbucket.try_build(
      "chromium",
      "linux",
      git_repo="https://chromium.googlesource.com/chromium/src",
  ...
  yield (
    api.test("basic_tags")
    + api.properties(patch_text="hihihi\nfoo:bar\nbam:baz", footer="foo")
    + api.step_data("parse description", api.json.output({"Foo": ["bar"]}))
    + api.step_data("parse description (2)", api.json.output({"Foo": ["bar"]}))
  )
