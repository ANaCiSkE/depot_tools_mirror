...
class OSXSDKTestApi(recipe_test_api.RecipeTestApi):
  ...
  def macos_version(
    self, major_minor: str = DEFAULT_MACOS_VERSION
    ...
    return self.step_data(
      "find macOS version", stdout=self.m.raw_io.output_text(major_minor)
    )

  def pick_sdk_version(
    self, sdk_version: str = "deadbeef"
  ) -> recipe_test_api.TestData:
