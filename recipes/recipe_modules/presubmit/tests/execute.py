...
def GenTests(api):
  ...
  yield (
    api.test("module_scheme_experiment")
    + api.runtime(is_experimental=False)
    + api.buildbucket.try_build(experiments={"presubmit.resultdb_module": 100})
    + api.cq(run_mode=api.cq.DRY_RUN)
    + api.post_process(post_process.StatusSuccess)
    + api.post_process(
  ...
  def _has_uploaded_findings(check, steps, expected_findings):
    ...
    findings = api.proto.decode(
      steps[step_name].logs["findings.json"], findings_pb.Findings, "JSONPB"
    ).findings
      ...
      api.json.output(
        {
          ...
          "warnings": [
            {
              "message": "Change takes 20 min to take effect after landing",
              "long_text": "",
              "items": [],
              "locations": [],
              "fatal": False,
            ...
            {
              "message": "typo!!",
              "long_text": "replace foo with bar",
              "items": [],
              "locations": [
                {
      ...
      [
        findings_pb.Finding(
          ...
          location=findings_pb.Location(
            gerrit_change_ref=gerrit_change_ref, file_path="path/to/file1"
          ),
          message="bug bug bug",
        ...
        findings_pb.Finding(
          category="chromium_presubmit",
          location=findings_pb.Location(
            gerrit_change_ref=gerrit_change_ref, file_path="path/to/file2"
          ),
          message="bug bug bug",
          severity_level=findings_pb.Finding.SEVERITY_LEVEL_ERROR,
        ),
        ...
        findings_pb.Finding(
          category="chromium_presubmit",
          location=findings_pb.Location(
            gerrit_change_ref=gerrit_change_ref, file_path="/COMMIT_MSG"
          ),
          message="cc abc@google.com",
    ...
    api.post_process(
      ...
      [
        findings_pb.Finding(
          category="chromium_presubmit",
          location=findings_pb.Location(
            gerrit_change_ref=gerrit_change_ref, file_path="path/to/file"
          ),
          message="this is a message",
          severity_level=findings_pb.Finding.SEVERITY_LEVEL_WARNING,
        ),
