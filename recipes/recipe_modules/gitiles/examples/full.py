...
def RunSteps(api):
  ...
  data = api.gitiles.download_file(
    url, "NONEXISTENT", attempts=1, accept_statuses=[404]
  )
...
def GenTests(api):
  yield (
    ...
    + api.step_data(
      "refs",
      api.gitiles.make_refs_test_data(
        "HEAD",
        "refs/heads/A",
        "refs/tags/B",
      ),
    ...
    + api.step_data(
      "gitiles log: refs/tags/B", api.gitiles.make_log_test_data("B")
    )
    + api.step_data(
    ...
    + api.step_data(
      "fetch main:OWNERS", api.gitiles.make_encoded_file("foobar")
    )
    + api.step_data(
      "fetch main:BYTES", api.gitiles.make_encoded_file_from_bytes(b"\xab")
    )
    + api.step_data("fetch main:NONEXISTENT", api.json.output({"value": None}))
    + api.step_data(
      (
