...
class Gitiles(recipe_api.RecipeApi):
  ...
  def _fetch(
    ...
    **kwargs,
    ...
    args = [
      "--json-file",
      self.m.json.output(add_json_log=add_json_log),
      "--url",
      url,
      "--format",
      fmt,
    ...
    if accept_statuses:
      args.extend(
        ["--accept-statuses", ",".join([str(s) for s in accept_statuses])]
      )
  ...
  def refs(self, url, step_name="refs", attempts=None):
    ...
    step_result = self._fetch(
      self.m.url.join(url, "+refs"), step_name, fmt="json", attempts=attempts
    )
  ...
  def log(
    ...
    **kwargs,
    ...
    step_result.presentation.step_text = "<br />%d commits fetched" % len(
      commits
    )
  ...
  def commit_log(self, url, commit, step_name=None, attempts=None):
    ...
    step_result = self._fetch(
      commit_url, step_name, attempts=attempts, fmt="json"
    )
  ...
  def download_archive(
    self, repository_url, destination, revision="refs/heads/main"
  ):
