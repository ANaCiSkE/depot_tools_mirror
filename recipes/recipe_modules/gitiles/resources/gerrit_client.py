...
def gitiles_get(parsed_url, handler, attempts):
  ...
  while True:
    ...
    logging.error(
      "Sleeping %d seconds before retry (%d/%d)...",
      retry_delay_seconds,
      attempt,
      attempts,
    )
...
def fetch_log_with_paging(query_params, limit, fetch):
  ...
  logging.debug(
    "fetched %d commits, next: %s.", len(commits), last_result.get("next")
  )
  return last_result
...
def main(arguments):
  ...
  if query_params.get("format"):
    parser.error(
      "URL must not contain format; use --format command line flag instead."
    )
...
def create_argparser():
  ...
  parser.add_argument(
    "--extract-to", help="Local path to extract archive url. Must not exist."
  )
  parser.add_argument(
