...
class RevisionFallbackChain(RevisionResolver):
  ...
  def resolve(self, properties):
    ...
    """
    return (
      properties.get("parent_got_revision")
      or properties.get("orig_revision")
      or properties.get("revision")
      or self._default
    )
...
class GclientApi(recipe_api.RecipeApi):
  ...
  def __call__(self, name, cmd, infra_step=True, **kwargs):
    ...
    with self.m.context(env=env, env_suffixes=env_suffixes):
      ...
      return self.m.step(prefix + name, cmd, infra_step=infra_step, **kwargs)
  ...
  def sync(self, cfg, extra_sync_flags=None, **kwargs):
    ...
    try:
      ...
      self(
        "sync",
        args + revisions + ["--output-json", self.m.json.output()],
  ...
  def checkout(
    self,
    gclient_config=None,
    revert=RevertOnTryserver,
    inject_parent_got_revision=True,
  ...
  def roll_deps(
    self, deps_path, dep_updates, strip_prefix_for_gitlink=None, test_data=None
  ):
    ...
    for dep, rev in dep_updates.items():
      ...
      if update_gitlink:
        ...
        gitlink_path = dep
        if strip_prefix_for_gitlink and gitlink_path.startswith(
          strip_prefix_for_gitlink
        ):
          # strip src/ from path
    ...
    self.m.gclient(
      "setdep", ["setdep", "--deps-file", deps_path] + dep_updates_args
    )
