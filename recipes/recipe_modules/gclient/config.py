...
def BaseConfig(
  USE_MIRROR=True, CACHE_DIR=None, deps_file=".DEPS.git", **_kwargs
):
  ...
  return ConfigGroup(
    ...
    revisions=Dict(value_type=(str, gclient_api.RevisionResolver), hidden=True),
    ...
    with_branch_heads=Single(
      bool, empty_val=False, required=False, hidden=True
    ),
    # Check out refs/tags.
...
def luci_py(c):
  ...
  c.revisions["infra"] = "refs/heads/main"
  c.revisions["infra/luci"] = gclient_api.RevisionFallbackChain(
    "refs/heads/main"
  )
  m = c.got_revision_mapping
  del m["infra"]
...
def recipes_py(c):
  c.revisions["infra"] = "refs/heads/main"
  c.revisions["infra/recipes-py"] = gclient_api.RevisionFallbackChain(
    "refs/heads/main"
  )
  m = c.got_revision_mapping
  del m["infra"]
...
def gerrit_plugins_chromium_binary_size(c):
  ...
  s.url = ChromiumGitURL(
    c, "infra", "gerrit-plugins", "chromium-binary-size.git"
  )
