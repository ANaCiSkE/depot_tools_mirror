...
def RunSteps(api):
  ...
  data = api.gerrit.create_gerrit_branch(
    host, project, branch, commit, allow_existent_branch=True, verbose=True
  )
  ...
  data = api.gerrit.create_gerrit_tag(
    host, project, "1.0", commit, verbose=True
  )
  ...
  json_data = api.gerrit.call_raw_api(
    host,
  ...
  related_changes = api.gerrit.get_related_changes(
    host, change="58478", revision="2", verbose=True
  )
  ...
  api.gerrit.get_file_content(
    host, change="123", path="path/to/file.txt", verbose=True
  )
...
def GenTests(api):
  ...
  def common_step_data():
    return (
      ...
      + api.step_data(
        "gerrit create_gerrit_tag (v8/v8 1.0)",
        api.gerrit.make_gerrit_create_tag_response_data(),
      )
      + api.step_data(
        "gerrit raw_create_tag",
        api.gerrit.make_gerrit_create_tag_response_data(),
      )
      + api.step_data(
        "gerrit create change at (v8/v8 main)",
        api.gerrit.update_files_response_data(),
      )
      + api.step_data(
        "verify the patchset exists on CL 91827.gerrit changes",
        api.gerrit.get_empty_changes_response_data(),
      )
      + api.step_data(
        "gerrit submit change 91827",
        api.gerrit.update_files_response_data(status="MERGED"),
      )
      + api.step_data(
        "gerrit get_gerrit_branch (v8/v8 main)",
      ...
      + api.step_data(
        "gerrit relatedchanges", api.gerrit.get_related_changes_response_data()
      )
      + api.step_data(
  ...
  yield (
    api.test("retry_success_on_first_retry", status="INFRA_FAILURE")
    ...
    + api.step_data(
      "gerrit changes (2)", api.gerrit.get_one_change_response_data()
    )
    + api.step_data(
      "gerrit changes empty query (2)",
      api.gerrit.get_empty_changes_response_data(),
    )
    + api.step_data(
      "gerrit changes empty query (3)",
      api.gerrit.get_empty_changes_response_data(),
    )
  ...
  yield (
    api.test("no_retry_wrong_zone", status="INFRA_FAILURE")
    + api.buildbucket.build(
      api.buildbucket.raw_swarming_build(
