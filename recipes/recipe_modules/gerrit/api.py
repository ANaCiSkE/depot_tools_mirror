...
class GerritApi(recipe_api.RecipeApi):
  ...
  def create_gerrit_branch(
    self, host, project, branch, commit, verbose=False, **kwargs
  ):
  ...
  def create_gerrit_tag(
    self, host, project, tag, commit, verbose=False, **kwargs
  ):
  ...
  def get_change_description(
    ...
    """
    ri = self.get_revision_info(
      host, change, patchset, timeout, step_test_data, verbose
    )
  ...
  def get_revision_info(
    ...
    step_test_data = step_test_data or (
      lambda: self.test_api.get_one_change_response_data(
  ...
  def get_changes(
    ...
    **kwargs,
    ...
    def _run_query(target_host):
      ...
      return self(
        step_name, args, step_test_data=step_test_data, **kwargs
      ).json.output
  ...
  def get_related_changes(
    self, host, change, revision="current", step_test_data=None, verbose=False
  ):
  ...
  def add_message(
    ...
    if automatic_attention_set_update is not None:
      args += [
        "--automatic-attention-set-update"
        if automatic_attention_set_update
  ...
  def update_files(
    ...
    """
    assert len(new_contents_by_file_path) > 0, (
      "The dict of file paths should not be empty."
    )
    create_command = [
      "createchange",
      "--host",
    ...
    step_result = self(
      "create change at (%s %s)" % (project, branch),
      create_command,
      step_test_data=step_test_data,
    )
    change = int(step_result.json.output.get("_number"))
    step_result.presentation.links["change %d" % change] = "%s/#/q/%d" % (
      host,
      change,
    )
    ...
    with self.m.step.nest("update contents in CL %d" % change):
      for path, content in new_contents_by_file_path.items():
        ...
        self.m.file.write_raw(
          "store the new content for %s" % path, _file, content
        )
        edit_file_command = [
          "changeedit",
          "--host",
    ...
    with self.m.step.nest("verify the patchset exists on CL %d" % change):
      ...
      while retries <= max_retries:
        try:
          if self.get_revision_info(
            host, change, patchset=NEW_PATCHSET_REVISION_NUMBER
          ):
            break
