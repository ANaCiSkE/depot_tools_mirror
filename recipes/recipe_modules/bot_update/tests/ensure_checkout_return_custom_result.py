...
def RunSteps(
  ...
  api.assertions.assertEqual(
    result.source_root.path, expected_checkout_dir / expected_source_root_name
  )

  if expected_patch_root_name is not None:
    api.assertions.assertEqual(result.patch_root.name, expected_patch_root_name)
    api.assertions.assertEqual(
      result.patch_root.path, expected_checkout_dir / expected_patch_root_name
    )
  else:
