...
def call(*args, **kwargs):  # pragma: no cover
  ...
  env.update(new_env)
  kwargs["env"] = env
  stale_process_duration = env.get(
    "STALE_PROCESS_DURATION", STALE_PROCESS_DURATION
  )
  ...
  if code:
    print(
      "%s ===Failed in %.1f mins of %s ==="
      % (datetime.now(), elapsed_time, " ".join(args))
    )
    print()
    raise SubprocessFailed(
      "%s failed with code %d in %s." % (" ".join(args), code, cwd),
      code,
      outval,
    )

  print(
    "%s ===Succeeded in %.1f mins of %s ==="
    % (datetime.now(), elapsed_time, " ".join(args))
  )
  print()
  return outval
...
def modify_solutions(input_solutions):
  ...
  for solution in solutions:
    ...
    if "safesync_url" in solution:
      print(
        "Removing safesync url %s from %s"
        % (solution["safesync_url"], parsed_path)
      )
...
def remove(target, cleanup_dir):
  ...
  dest = path.join(
    cleanup_dir, "%s_%s" % (path.basename(target), uuid.uuid4().hex)
  )
...
def ensure_no_checkout(dir_names, cleanup_dir):
  ...
  build_dir = os.getcwd()
  has_checkout = any(
    path.exists(path.join(build_dir, dir_name, ".git"))
  ...
  if has_checkout:
    for filename in os.listdir(build_dir):
      ...
      print(
        ".git detected in checkout, deleting %s..." % deletion_target,
      )
...
def get_total_disk_space():
  ...
  if sys.platform.startswith("win"):
    ...
    ret = ctypes.windll.kernel32.GetDiskFreeSpaceExW(
      cwd, ctypes.byref(_), ctypes.byref(total), ctypes.byref(free)
    )
    if ret == 0:
...
def _has_in_git_cache(revision_sha1, refs, git_cache_dir, url):
  ...
  try:
    mirror_dir = git(
      "cache", "exists", "--quiet", "--cache-dir", git_cache_dir, url
    ).strip()
    if revision_sha1:
...
def _set_git_config(fn):
  ...
  def wrapper(*args, **kwargs):
    with (
      git_config_if_not_set("user.name", "chrome-bot"),
...
def get_commit_position(git_path, revision="HEAD"):
  ...
  value = footer_map.get(COMMIT_POSITION_FOOTER_KEY) or footer_map.get(
    COMMIT_ORIGINAL_POSITION_FOOTER_KEY
  )
  if value:
    return value
  return None
...
def ensure_checkout(
  ...
  gc_revisions = {
    dirname: rev
    for dirname, rev in revisions.items()
...
def parse_args():
  ...
  parse.add_option(
    "--experiments", help="Comma separated list of experiments to enable"
  )
  ...
  parse.add_option(
    ...
    help=('Path to a json file of the form {"property_name": "path/to/repo/"}'),
  )
  parse.add_option(
    "--revision",
    action="append",
  ...
  parse.add_option(
    ...
    action="store_true",
    help=(
      "Don't fetch tags from the server for the git checkout. "
  ...
  parse.add_option(
    ...
    action="store_true",
    help=(
      "Enforce a new fetch to refresh the git cache, even if the "
      "solution revision passed in already exists in the current "
      "git cache."
    ),
  )
  parse.add_option("--download_topics", action="store_true")
  parse.add_option(
    "--clean-ignored",
    action="store_true",
  ...
  parse.add_option(
    "--clobber", action="store_true", help="Delete checkout first, always"
  )
  parse.add_option(
    "--output_json", help="Output JSON information into a specified file"
  )
  parse.add_option(
    "--refs",
    action="append",
    ...
    try:
      ...
      if any(s in stderr.lower() for s in signatures):
        ...
        if git_cache_dir and os.path.exists(git_cache_dir):
          print(
            f"Wiping entire git cache directory to ensure recovery from corruption: {git_cache_dir}"
          )
          try:
