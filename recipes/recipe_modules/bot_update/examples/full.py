...
def GenTests(api):
  ...
  def ci_build(**kwargs):
    ...
    return api.buildbucket.ci_build(
      "chromium/src", "ci", "linux", **kwargs
    ) + api.properties(patch=False)

  yield (api.test("basic") + ci_build())
  yield (
  ...
  yield (
    api.test("unrecognized_commit_repo", status="INFRA_FAILURE")
  ...
  yield (
    ...
    + ci_build()
    + api.properties(with_branch_heads=True, suffix="with branch heads")
  )
  ...
  yield (api.test("refs") + api.properties(refs=["+refs/change/1/2/333"]))
  yield (
    api.test("tryjob_fail", status="INFRA_FAILURE")
    + try_build()
  ...
  yield (api.test("clobber") + api.properties(clobber=1))
  yield (
    api.test("reset_root_solution_revision")
    + api.properties(
      root_solution_revision=api.bot_update.gen_revision("fake-revision")
    )
  )
  yield (api.test("gerrit_no_reset") + api.properties(gerrit_no_reset=1))
  yield (
  ...
  yield (api.test("origin_main") + ci_build(revision="origin/main"))
  ...
  yield (
    api.test("stale-process-duration-override")
    + api.properties(
      **{
  ...
  def check_revisions(check, output_json):
    preconditions_valid = check(
      output_json["fixed_revisions"]["infra"] == "HEAD"
    ) and check(
      output_json["fixed_revisions"]["src"]
    ...
    check(
      manifest["src"]["revision"] == "2d72510e447ab60a9728aeea2362d8be2cbd7789"
    )
    # An empty revision specified for a project where there is not a fixed
    # revision will generate a revision for 'HEAD'
    check(
      manifest["src/foo"]["revision"] == api.bot_update.gen_revision("src/foo")
    )
    ...
    check(
      manifest["src/v8"]["revision"]
      == api.bot_update.gen_revision("src/v8@refs/bar")
    )
  ...
  yield api.test(
    ...
    api.bot_update.post_check_output_json(
      "bot_update (without patch)", check_revisions
    ),
    api.post_process(post_process.DropExpectation),
  )
  ...
  def check_revisions_ref_revision_syntax(check, output_json):
    # Precondition check
    if not check(
      output_json["fixed_revisions"]["src"] == "refs/heads/foo:" + "a" * 40
    ):
      return  # pragma: no cover
