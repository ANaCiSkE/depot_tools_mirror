    ...
    def _FetchChangeDetail(self, issue):
        ...
        try:
            ...
            if e.http_status == 404:
                raise Exception(
                    "Either Gerrit issue %s doesn't exist, or "
    ...
    def GetChangeDescription(self, issue, patchset=None):
        ...
        if patchset is not None:
            for rev, rev_info in info["revisions"].items():
                ...
                raise Exception(
                    "patchset %s doesn't exist in issue %s" % (patchset, issue)
                )
        else:
            ...
            if self.persistent_cache:
                ...
                if self.code_owners_enabled is None:
                    self.code_owners_enabled = (
                        gerrit_util.IsCodeOwnersEnabledOnRepo(
                            self.host, self.project
                        )
                    )
                    self.persistent_cache.setBoolean(
                        key, self.code_owners_enabled
                    )
            else:
                self.code_owners_enabled = (
                    gerrit_util.IsCodeOwnersEnabledOnRepo(
...
class InputApi(object):
    ...
    def AffectedTestableFiles(self, include_deletes=None, **kwargs):
        ...
        if include_deletes is not None:
            warn(
                "AffectedTestableFiles(include_deletes=%s)"
    ...
    def FilterSourceFile(
        ...
        return Find(affected_file, files_to_check) and not Find(
            affected_file, files_to_skip
        )
...
class Change(object):
    ...
    def __init__(
        ...
        assert all(
            (isinstance(f, (list, tuple)) and len(f) == 2) for f in files
        ), files

        diff_cache = self._diff_cache()
        self._affected_files = [
            self._AFFECTED_FILES(
                path, action.strip(), self._local_root, diff_cache
            )
    ...
    def AffectedSubmodules(self):
        ...
        return [
            af
            for af in self._affected_files
    ...
    def RightHandSideLines(self):
        ...
        return _RightHandSideLinesImpl(
            x
            for x in self.AffectedFiles(include_deletes=False)
            if x.IsTestableFile()
        )
...
class GetPostUploadExecuter(object):
    ...
    def _execute_with_local_working_directory(
        self, script_text, presubmit_dir, presubmit_path
    ):
        context = {}
        try:
            exec(
        ...
        def _progress_loop(event):
            while not event.is_set():
                if event.wait(timeout=30):
                    return
                sys.stdout.write(
                    f"Still running {function_name} after "
...
def _parse_change(parser, options):
    ...
    if change_scm == "diff" and not (
        options.files
        or options.all_files
    ...
    if options.files:
        ...
        if change_scm == "git":
            change_files = scm.GIT.CaptureStatus(
                options.root, options.upstream or None, ignore_submodules=False
            )
        else:
    ...
    try:
        for method_name in method_names:
            if not hasattr(presubmit_canned_checks, method_name):
                logging.warning(
                    'Skipping unknown "canned" check %s' % method_name
                )
                continue
            filtered[method_name] = getattr(
                presubmit_canned_checks, method_name
            )
...
def main(argv=None):
    ...
    parser.add_argument(
        "-r", "--recursive", action="store_true", help="Act recursively."
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    desc.add_argument(
        "--description", default="", help="The change description."
    )
    desc.add_argument(
        "--description_file", help="File to read change description from."
    )
    ...
    parser.add_argument(
        "--end_commit",
        default="HEAD",
        help="The commit to diff against upstream. By default this is HEAD.",
    )
    ...
    parser.add_argument(
        "--gerrit_fetch", action="store_true", help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--parallel",
        action="store_true",
    ...
    log_format = (
        "[%(levelname).1s%(asctime)s %(process)d %(thread)d "
    ...
    try:
        ...
        print(
            "depot_tools version: %s" % utils.depot_tools_version(),
            file=sys.stderr,
        )
        return 2
