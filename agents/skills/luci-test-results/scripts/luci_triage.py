...
def main():
    ...
    p.add_argument(
        "--all", action="store_true", help="Show all builds, not just failures"
    )
    ...
    p.add_argument(
        "--raw", action="store_true", help="Return full log without filtering"
    )
    ...
    if args.command == "resolve-build-id":
        ...
        print(
            json.dumps(
                find_cl_builds(args.cl, args.patchset, args.host, args.all),
                indent=2,
            )
        )
