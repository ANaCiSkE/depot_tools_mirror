...
class Hook(object):
    ...
    @staticmethod
    def from_dict(
        d, variables=None, verbose=False, conditions=None, cwd_base=None
    ):
    ...
    def __init__(
        ...
        if isinstance(url, str):
            ...
            raise gclient_utils.Error(
                ("dependency url must be either string or None, instead of %s")
...
class Dependency(gclient_utils.WorkItem, DependencySettings):
    ...
    def _OverrideUrl(self):
        ...
        if self.url is None:
            logging.info(
                "Dependency(%s)._OverrideUrl(None) -> None", self._name
            )
            return
    ...
    def PinToActualRevision(self):
        ...
        if os.path.isdir(scm.checkout_path):
            revision = scm.revinfo(None, None, None)
            url = "%s@%s" % (
                gclient_utils.SplitUrlRevision(self.url)[0],
                revision,
            )
        self.set_url(url)
    ...
    def requirements(self):
        ...
        if self.parent and self.parent.parent and not self.parent.parent.parent:
            requirements |= set(
                i.name for i in self.root.dependencies if i.name
            )

        if self.name:
            requirements |= set(
                obj.name
                for obj in self.root.subtree(False)
                if (
                    obj is not self
                    and obj.name
        ...
        logging.info(
            "Dependency(%s).requirements = %s" % (self.name, requirements)
        )
        return requirements
    ...
    def _postprocess_deps(self, deps, rel_prefix):
        ...
        if not self._should_sync:
            ...
            for dep_name, dep_info in self.custom_deps.items():
                ...
                if (
                    dep_name not in processed_deps
                    and dep_info
        ...
        for d, url in processed_deps.items():
            ...
            # which uses slashes.
            rel_deps[
                os.path.normpath(os.path.join(rel_prefix, d)).replace(
                    os.path.sep, "/"
                )
            ] = url
    ...
    def _deps_to_objects(self, deps, use_relative_paths):
        ...
        for name, dep_value in deps.items():
            ...
            if dep_type == "cipd":
                ...
                object_name_set = {
                    o["object_name"] for o in dep_value["objects"]
                }
                if len(object_name_set) != len(dep_value["objects"]):
                    raise Exception(
                        "Duplicate object names detected in {} GCS "
                ...
                for obj in dep_value["objects"]:
                    ...
                    # TODO(b/345321320): Remove when non_git_sources are properly supported.
                    if gclient_utils.IsEnvCog() and (
                        not merged_condition
                        or "non_git_source" not in merged_condition
                    ):
                        continue
    ...
    def ParseDepsFile(self):
        ...
        if "allowed_hosts" in local_scope:
            ...
            if not self._allowed_hosts:
                logging.warning(
                    "allowed_hosts is specified but empty %s",
                    self._allowed_hosts,
                )
                raise gclient_utils.Error(
        ...
        if "recursedeps" in local_scope:
            ...
            if rel_prefix:
                logging.warning(
                    "Updating recursedeps by prepending %s.", rel_prefix
                )
                rel_deps = {}
                for depname, options in self.recursedeps.items():
                    rel_deps[
                        os.path.normpath(
                            os.path.join(rel_prefix, depname)
                        ).replace(os.path.sep, "/")
                    ] = options
        ...
        if self._gn_args_from:
            assert (
                self.recursedeps and self._gn_args_from in self.recursedeps
        ...
        if self.git_dependencies_state != gclient_eval.DEPS:
            ...
            self._dependency_index_state = (
                self.CreateSCM().GetSubmoduleStateFromIndex()
            )
        ...
        if local_scope.get("use_relative_hooks", False):
            print(
                "use_relative_hooks is deprecated, please remove it from "
        ...
        self.add_dependencies_and_close(
            deps_to_add, hooks_to_run, hooks_cwd=hooks_cwd
        )
    ...
    def ParseGitSubmodules(self):
        ...
        if not os.path.isfile(filepath):
            logging.warning(
                "ParseGitSubmodules(): No .gitmodules found at %s", filepath
            )
            return {}

        # Get .gitmodules fields
        gitmodules_entries = subprocess2.check_output(
            ["git", "config", "--file", filepath, "-l"]
        ).decode("utf-8")
        ...
        for module in gitmodules.values():
            ...
            submodules[path] = {
                "dep_type": "git",
                "url": "{}@{}".format(
                    module["url"], commit_hashes.get(module["path"], "0000000")
                ),
            }
    ...
    def findDepsFromNotAllowedHosts(self):
        ...
        for dep in self._dependencies:
            ...
            if isinstance(dep.url, str):
                ...
                if (
                    parsed_url.netloc
                    and parsed_url.netloc not in self._allowed_hosts
                ):
                    bad_deps.append(dep)
        return bad_deps
    ...
    def run(
        ...
        if run_scm and self.url:
            ...
            if command != "update" or self.GetScmName() != "git":
                ...
                try:
                    ...
                    if (
                        self.parent
                        and self.parent.known_dependency_diff is not None
                    ):
                        if self._use_relative_paths:
                            path = self.name
                        else:
                        ...
                        if path in self.parent.dependency_index_state:
                            current_revision = (
                                self.parent.dependency_index_state[path]
                            )
                        if path in self.parent.known_dependency_diff:
                            current_revision = (
                                self.parent.known_dependency_diff[path][1]
                            )
        ...
        if skip_sync_revisions:
            ...
            self._should_sync = (
                skip_sync_rev is None
                or self._used_scm.check_diff(skip_sync_rev, files=["DEPS"])
            )
            if not self._should_sync:
                logging.debug(
                ...
                logging.debug(
                    "DEPS changes detected for %s since last sync at "
                    "%s. Not skipping deps sync" % (self.name, skip_sync_rev)
                )
        ...
        if command == "recurse":
            ...
            if not options.scm or scm in options.scm:
                ...
                if self.url is None:
                    print(
                        "Skipped omitted dependency %s" % cwd, file=sys.stderr
                    )
...
class GitDependency(Dependency):
    ...
    def updateProtocol(url, protocol):
        ...
        if (
            not url
            or not protocol
            or not re.match("([a-z]+)://", url)
            or re.match("file://", url)
        ):
            return url
    ...
    def CreateSCM(self, out_cb=None):
        ...
        if GitDependency._scm is None:
            if os.path.exists(
                os.path.join(self.root.root_dir, self.name, ".jj")
            ):
...
"""
    ...
    def SetConfig(self, content):
        ...
        if not target_cpu and config_dict.get("target_cpu_only", False):
            raise gclient_utils.Error(
                "Can't use target_cpu_only if target_cpu is not specified"
            )
        ...
        for s in config_dict.get("solutions", []):
            try:
                ...
                raise gclient_utils.Error(
                    "Invalid .gclient file. Solution is incomplete: %s" % s
                )
        ...
        self.add_dependencies_and_close(
            deps_to_add, config_dict.get("hooks", [])
        )
    ...
    def LoadCurrentConfig(options):
        ...
        if options.spec:
            ...
            if options.verbose:
                print(
                    "Looking for %s starting from %s\n"
                    % (options.config_filename, os.getcwd())
                )
            path = gclient_paths.FindGclientRoot(
                os.getcwd(), options.config_filename
            )
            if not path:
                if options.verbose:
    ...
    def _ReadEntries(self):
        ...
        if not os.path.exists(filename):
            return {}
        scope = gclient_eval.ParseLocalConfig(
            gclient_utils.FileRead(filename), filename
        )
    ...
    def _InstallPreCommitHook(self):
        ...
        gclient_hook_path = os.path.join(
            DEPOT_TOOLS_DIR, "hooks", "pre-commit.py"
        ).replace("\\", "\\\\")
    ...
    def _RemoveUnversionedGitDirs(self):
        ...
        entry_names_and_sync = [
            (i.name, i._should_sync) for i in self.root.subtree(False) if i.url
        ]
        entries = []
        ...
        no_sync_entries = [
            name
            for name, should_sync in entry_names_and_sync
            if not should_sync
        ]
        ...
        for entry in sorted(read_entries, reverse=True):
            ...
            if (
                ...
                and (not any(path.startswith(entry + "/") for path in entries))
                ...
                if not self._options.delete_unversioned_trees or (
                    ...
                    if (
                        modified_files
                        and self._options.delete_unversioned_trees
                    ):
                        print(
    ...
    def RunOnDeps(
        self, command, args, ignore_requirements=False, progress=True
    ):
        ...
        if custom_vars:
            self._WriteFileContents(
                PREVIOUS_CUSTOM_VARS_FILE, json.dumps(custom_vars)
            )
        ...
        if (
            recommendation
            and os.environ.get("GCLIENT_SUPPRESS_GIT_VERSION_WARNING") != "1"
        ):
            message = (
                f"{recommendation}\n"
        ...
        if command == "update" or (command == "runhooks" and self._IsCog()):
            for dependency in self.dependencies:
                ...
                if gn_args_dep._gn_args_from:
                    deps_map = {
                        dep.name: dep for dep in gn_args_dep.dependencies
                    }
    ...
    def PrintRevInfo(self):
        ...
        def ShouldPrintRevision(dep):
            return not self._options.filter or dep.FuzzyMatchUrl(
        ...
        if self._options.snapshot:
            ...
            if self._options.output_json == "-":
                ...
                print(
                    self.DEFAULT_SNAPSHOT_FILE_TEXT
                    % {
            ...
            if self._options.output_json:
                ...
                if self._options.output_json == "-":
                    print(
                        json.dumps(
                            json_output, indent=2, separators=(",", ": ")
                        )
                    )
                else:
                    with open(self._options.output_json, "w") as f:
...
class GcsDependency(Dependency):
    ...
    def __init__(
        ...
        super(GcsDependency, self).__init__(
            parent=parent,
    ...
    def DownloadGoogleStorage(self):
        ...
        if os.getenv("GCLIENT_TEST") == "1":
            if "no-extract" in self.artifact_output_file:
                ...
                copy_dir = os.path.join(
                    tmpdir, self.gcs_file_name, "extracted_dir"
                )
            ...
            code, _, err = gsutil.check_call(
                "cp", self.url, self.artifact_output_file
            )
            if code and err:
        ...
        if calculated_sha256sum != self.sha256sum:
            raise Exception(
                "sha256sum does not match calculated hash. "
        ...
        if calculated_size_bytes != self.size_bytes:
            raise Exception(
                "size_bytes does not match calculated size bytes. "
                "{original} vs {calculated}".format(
...
class CipdDependency(Dependency):
    ...
    def __init__(
        ...
        if dep_value.get("version_file"):
            fp = os.path.join(
                parent.root.root_dir, parent.name, dep_value["version_file"]
            )
...
class Flattener(object):
    ...
    def _flatten(self, pin_all_deps=False):
        ...
        def add_deps_file(dep):
            ...
            if not os.path.exists(deps_path):
                ...
                deps_path = os.path.join(
                    self._client.root_dir, dep.name, deps_file
                )
                if not os.path.exists(deps_path):
                    return
            assert dep.url
        ...
        gn_args_dep = self._deps.get(
            self._client.dependencies[0]._gn_args_from,
    ...
    def _flatten_dep(self, dep):
        ...
        self._pre_deps_hooks.extend(
            [(dep, hook) for hook in dep.pre_deps_hooks]
        )
...
def CMDgitmodules(parser, args):
    ...
    parser.add_option(
        "--deps-file",
        help="name of the deps file to parse for git dependency paths and commits.",
        default="DEPS",
    )
    parser.add_option(
        "--skip-dep",
    ...
    with open(options.output_gitmodules, "w", newline="") as f:
        for path, dep in ls.get("deps").items():
            ...
            try:
                ...
                logging.error(
                    "error on %s; %s, not adding it", path, dep["url"]
                )
                continue
    ...
    print(
        ".gitmodules and gitlinks updated. Please check `git diff --staged` "
...
def CMDflatten(parser, args):
    ...
    parser.add_option(
        "--output-deps-files",
        help=(
            "Path to the output metadata about DEPS files referenced by "
            "recursedeps."
        ),
    )
    parser.add_option(
        ...
        action="store_true",
        help=(
            "Pin all deps, even if not pinned in DEPS. CAVEAT: only does so "
            "for checked out deps, NOT deps_os."
        ),
    )
    parser.add_option(
        "--deps-graph-file", help="Provide a path for the output graph file"
    )
    options, args = parser.parse_args(args)
    ...
    deps_files = [
        {"url": d[0], "deps_file": d[1], "hierarchy": d[2]}
...
def CMDconfig(parser, args):
    ...
    parser.add_option(
        "--name", help="overrides the default name for the solution"
    )
    parser.add_option(
        "--deps-file",
        default="DEPS",
    ...
    if options.output_config_file:
        setattr(
            options, "config_filename", getattr(options, "output_config_file")
        )
    if (
        (options.spec and args)
        or len(args) > 2
    ...
    if (
        options.cache_dir is not UNSET_CACHE_DIR
...
def CMDsync(parser, args):
    ...
    parser.add_option(
        "--ignore_locks", action="store_true", help="No longer used."
    )
    parser.add_option(
        "--break_repo_locks", action="store_true", help="No longer used."
    )
    parser.add_option(
        "--lock_timeout",
        type="int",
...
def CMDgetconfig(parser, args):
    ...
    if missing_keys:
        for key in missing_keys:
            print(
                f"Key {key} not found in {options.config_filename} config",
                file=sys.stderr,
            )
        return 2
...
def CMDgetdep(parser, args):
    ...
    local_scope = gclient_eval.Exec(
        contents, options.deps_file, builtin_vars=builtin_vars
    )
    ...
    commits = {}
    if (
        local_scope.get("git_dependencies") == gclient_eval.SUBMODULES
        and options.getdep_revisions
    ):
        commits.update(
...
def CMDsetdep(parser, args):
    ...
    parser.add_option(
        ...
        metavar="VAR=VAL",
        default=[],
        help="Sets a variable to the given value with the format name=value.",
    )
    parser.add_option(
        "-r",
        "--revision",
    ...
    if (
        not is_cog
        and "git_dependencies" in local_scope
        ...
        if p.returncode != 0:
            print(
                "Warning: gitlinks won't be updated because computing "
    ...
    for revision in options.setdep_revisions:
        ...
        if ":" in name:
            ...
            if "git_dependencies" not in local_scope or local_scope[
                "git_dependencies"
            ] in (gclient_eval.DEPS, gclient_eval.SYNC):
            ...
            if (
                ...
                if (
                    ...
                    if delta_path:
                        prefix_length = (
                            len(delta_path.replace(os.path.sep, "/")) + 1
                        )
    ...
    if git_modules:
        ...
        print(
            "Changes have been staged. See changes with `git status`.\n"
...
def CMDverify(parser, args):
    ...
    for dep in client.dependencies:
        ...
        if not bad_deps:
            continue
        print(
            "There are deps from not allowed hosts in file %s" % dep.deps_file
        )
...
def path_contains_tilde():
    for element in os.environ["PATH"].split(os.pathsep):
        if (
            element.startswith("~")
            and os.path.abspath(os.path.realpath(os.path.expanduser(element)))
            == DEPOT_TOOLS_DIR
        ):
            return True
    return False
