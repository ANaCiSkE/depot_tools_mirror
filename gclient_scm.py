...
class DiffFiltererWrapper(object):
    ...
    def Filter(self, line):
        if line.startswith(self.index_string):
            ...
            if line.startswith(self.original_prefix) or line.startswith(
                self.working_prefix
            ):
                line = self._Replace(line)
...
class GitDiffFilterer(DiffFiltererWrapper):
    ...
    def _Replace(self, line):
        return re.sub(
            "[a|b]/" + self._current_file, self._replacement_file, line
        )
...
class SCMWrapper(object):
    ...
    def GetActualRemoteURL(self, options):
        ...
        if os.path.exists(os.path.join(self.checkout_path, ".git")):
            ...
            if (
                mirror
                and mirror.exists()
                and mirror.mirror_path.replace("\\", "/")
    ...
    def _DeleteOrMove(self, force):
        ...
        if force and os.environ.get("CHROME_HEADLESS") == "1":
            self.Print(
                "_____ Conflicting directory found in %s. Removing."
                % checkout_path
            )
            gclient_utils.AddWarning(
                "Conflicting directory %s deleted." % checkout_path
            )
            ...
            bad_scm_dir = os.path.join(
                self._root_dir, bad_scm_dir_name, os.path.dirname(relpath)
            )
            ...
            dest_path = tempfile.mkdtemp(
                prefix=os.path.basename(relpath), dir=bad_scm_dir
            )
            self.Print(
            ...
            gclient_utils.AddWarning(
                "Conflicting directory %s moved to %s."
                % (checkout_path, dest_path)
            )
...
def _canonical_git_repo(url):
    ...
    return _canonical_git_url(
        url.replace("googlesource.com/a/", "googlesource.com/")
    )
...
class GitWrapper(SCMWrapper):
    ...
    def __init__(self, url=None, *args, **kwargs):
        ...
        if url and (
            url.startswith("git+http://") or url.startswith("git+https://")
        ):
            url = url[4:]
    ...
    def pack(self, _options, _args, _file_list):
        ...
        gclient_utils.CheckCallAndFilter(
            ...
            filter_fn=GitDiffFilterer(
                self.relpath, print_func=self.Print
            ).Filter,
        )
                ...
                for filename in filenames:
                    if filename.endswith(".lock"):
                        ...
                        try:
                            ...
                            self.Print(
                                "FAILED to break lock: %s: %s" % (to_break, ex)
                            )
                            raise
    ...
    def _download_topics(self, patch_rev, googlesource_url):
        ...
        tokens = re.search(
            r"//(.+).googlesource.com/(.+?)(?:\.git)?$", googlesource_url
        )
        ...
        for c in changes_with_same_topic:
            if str(c["_number"]) == change:
                ...
                continue
            self.Print(
                "Found CL %d with the topic name %s" % (c["_number"], topic)
            )
    ...
    def apply_patch_ref(
        self, patch_repo, patch_rev, target_rev, options, file_list
    ):
    ...
    """
        ...
        if target_rev.startswith(("refs/heads/", "refs/branch-heads")):
            ...
            self.Print(
                "Trying the corresponding remote ref for %r: %r\n"
            ...
            if mirror:
                rev_type = (
                    "branch" if target_rev.startswith("refs/") else "hash"
                )
                self._UpdateMirrorIfNotContains(
                    mirror, options, rev_type, target_rev
                )
        ...
        for pr in patch_revs_to_process:
            ...
            if not options.rebase_patch_ref:
                ...
                try:
                    if scm.GIT.IsAncestor(
                        pr, target_rev, cwd=self.checkout_path
                    ):
                    ...
                    self.Print(
                        "Revision to patch was %r @ %r." % (patch_repo, pr)
                    )
                    self.Print(
                        "Tried to cherrypick %r .. %r on top of %r."
                        % (target_rev, pr, base_rev)
                    )
                    self.Print("Current dir is %r" % self.checkout_path)
                    self.Print(
                        "git returned non-zero exit status %s:\n%s"
        ...
        def wrapper(*args):
            ...
            if os.path.exists(os.path.join(checkout_path, ".git")):
                ...
                blame_ignore_revs_exists = os.path.isfile(
                    os.path.join(
                        checkout_path, git_common.GIT_BLAME_IGNORE_REV_FILE
                    )
                )
                ...
                if not ignore_submodules:
                    ...
                    if (
                        os.environ.get("GCLIENT_SUPPRESS_SUBMODULE_WARNING")
                        != "1"
                    ):
                        gclient_utils.AddWarning(warning_message)

                if (
                    scm.GIT.GetConfig(checkout_path, "fetch.recursesubmodules")
                    != "off"
                ):
                    config_updates.append(("fetch.recurseSubmodules", "off"))

                if (
                    scm.GIT.GetConfig(checkout_path, "push.recursesubmodules")
                    != "off"
                ):
                ...
                for update in config_updates:
                    scm.GIT.SetConfig(
                        checkout_path, update[0], update[1], modify_all=True
                    )
    ...
    def update(self, options, args, file_list):
        ...
        if args:
            raise gclient_utils.Error(
                "Unsupported argument(s): %s" % ",".join(args)
            )
        ...
        if not revision:
            ...
            if git_dir_exists and url != "git://foo":
                ...
                current_url = scm.GIT.GetConfig(
                    self.checkout_path, f"remote.{self.remote}.url", default=""
                )
            ...
            if branch:
                ...
                revision = scm.GIT.GetRemoteHeadRef(
                    self.checkout_path,
                    self.url,
                    self.remote,
        ...
        if options.verbose:
            self.Print(
                "_____ %s at %s" % (self.relpath, revision), timestamp=False
            )
            verbose = ["--verbose"]
            printed_path = True
        ...
        if not os.path.exists(self.checkout_path) or (
            ...
            if mirror:
                self._UpdateMirrorIfNotContains(
                    mirror, options, rev_type, revision
                )
            try:
        ...
        if not managed:
            self._SetFetchConfig(options)
            self.Print(
                "________ unmanaged solution; skipping %s" % self.relpath
            )
            return self._Capture(["rev-parse", "--verify", "HEAD"])
        ...
        if (
            _canonical_git_url(current_url) != _canonical_git_url(url)
            and url != "git://foo"
        ):
            self.Print(
                "_____ switching %s from %s to new upstream %s"
            ...
            if mirror:
                if git_cache.Mirror.CacheDirToUrl(
                    current_url.rstrip("/")
                ) == git_cache.Mirror.CacheDirToUrl(url.rstrip("/")):
        ...
        if cur_branch is None:
            ...
            if not upstream_branch or not upstream_branch.startswith(
                ...
                raise gclient_utils.Error(
                    "Invalid Upstream: %s" % upstream_branch
                )
        ...
        if not scm.GIT.IsValidRevision(
            ...
            if not scm.GIT.IsValidRevision(
                ...
                remote_output = scm.GIT.Capture(
                    ["remote"] + verbose + ["update"], cwd=self.checkout_path
                )
                if verbose:
        ...
        if current_type == "detached":
            ...
            if force_switch:
                self.Print(
                    "Switching upstream branch from %s to %s"
                    % (upstream_branch, new_base)
                )
            ...
            try:
                ...
                rebase_files = []
                if re.search(
                    b"fatal: Not possible to fast-forward, aborting.", e.stderr
                ):
                    if not printed_path:
                        self.Print(
                            "_____ %s at %s" % (self.relpath, revision),
                            timestamp=False,
                        )
                        printed_path = True
                    while True:
                    ...
                    logging.error(
                        "Error during fast-forward merge in %s!" % self.relpath
                    )
        ...
        if verbose:
            self.Print(
                f"Checked out revision {current_revision}", timestamp=False
            )
    ...
    def revert(self, options, _args, file_list):
        ...
        if deps_revision.startswith("refs/heads/"):
            deps_revision = deps_revision.replace(
                "refs/heads/", self.remote + "/"
            )
        try:
    ...
    def GetGitBackupDirPath(self):
        ...
        return (
            os.path.join(
                self._root_dir, "old_" + self.relpath.replace(os.sep, "_")
            )
            + ".git"
        )
    ...
    def _AskForData(self, prompt, options):
        if options.jobs > 1:
            self.Print(prompt)
            raise gclient_utils.Error(
                "Background task requires input. Rerun "
    ...
    def _IsRebasing(self):
        ...
        return os.path.isdir(os.path.join(g, "rebase-merge")) or os.path.isdir(
    ...
    def _Checkout(self, options, ref, force=False, quiet=None):
        ...
        with trace_utils.trace("git._Checkout", cat="scm", args={"ref": ref}):
            if not ref:
                raise gclient_utils.Error(
                    "A ref must be specified for checkout"
                )
            if ref.startswith("-"):
                raise gclient_utils.Error(
    ...
    def _Fetch(
        ...
        with trace_utils.trace(
            ...
            if not target_remote:
                raise gclient_utils.Error(
                    "A remote must be specified for fetch"
                )
...
class CipdRoot(object):
    ...
    def add_package(self, subdir, package, version):
        ...
        with self._mutator_lock:
            cipd_package = CipdPackage(
                package, version, not self._packages_by_subdir[subdir]
            )
    ...
    def _create_resolved_file(self):
        return tempfile.NamedTemporaryFile(
            suffix=".resolved", delete=False, mode="wb"
        )
        ...
        with self._mutator_lock:
            with self._create_resolved_file() as output_file:
                with self._create_ensure_file_for_resolve() as ensure_file:
                    ...
                    gclient_utils.CheckCallAndFilter(
                        cmd, print_stdout=False, show_header=False
                    )
                ...
                if len(path_split) > 1:
                    ...
                    if src_path in resolved:
                        for resolved_package in resolved[src_path]:
                            if package == resolved_package.get("pin", {}).get(
                                "package"
                            ):
                                return resolved_package.get("pin", {}).get(
                                    "instance_id"
                                )
...
class GcsRoot(object):
    ...
    def clobber_migration_files(self, entry_directory):
        files = glob.glob(
            os.path.join(entry_directory, ".*_is_first_class_gcs")
        )
        for f in files:
            os.remove(f)
...
class GcsWrapper(SCMWrapper):
    ...
    def __init__(
        self, url=None, root_dir=None, relpath=None, out_fh=None, out_cb=None
    ):
        super(GcsWrapper, self).__init__(
            url=url,
            root_dir=root_dir,
            relpath=relpath,
            out_fh=out_fh,
