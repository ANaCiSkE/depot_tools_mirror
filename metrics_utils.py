...
KNOWN_HTTP_PATHS = {
    ...
    "changes/revisions/commit": re.compile(
        r"(/a)?/changes/.*/revisions/.*/commit"
    ),
    "changes/revisions/review": re.compile(
        r"(/a)?/changes/.*/revisions/.*/review"
    ),
...
def get_git_version():
    ...
    p = subprocess2.Popen(
        ["git", "--version"], stdout=subprocess2.PIPE, stderr=subprocess2.PIPE
    )
