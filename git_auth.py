...
class UserInterface(object):
    ...
    def read_line(
        self, prompt: str, *, check: _InputChecker = _check_any
    ) -> str:
...
class ConfigWizard(object):
    ...
    def _run_outside_repo(self) -> None:
        ...
        if not self._read_yn(
            "Set up commonly used Gerrit hosts?", default=True
        ):
    ...
    def _configure_sso(
        self, parts: urllib.parse.SplitResult, *, scope: scm.GitConfigScope
    ) -> None:
        if parts.scheme == "sso":
    ...
    def _check_gitcookies(self) -> _GitcookiesSituation:
        ...
        divergent_cookiefiles = (
            gitcookies_exists
            and cookiefile_exists
            and not os.path.samefile(gitcookies, cookiefile)
        )
    ...
    def _check_gce(self):
        ...
        fallback_msg = (
            "Add `export SKIP_GCE_AUTH_FOR_GIT=1` to your .bashrc or similar."
        )
        if self._is_windows():
    ...
    def _check_global_email(self) -> str:
        ...
        """
        email = (
            scm.GIT.GetConfig(os.getcwd(), "user.email", scope="global") or ""
        )
        if email:
        ...
        if not name:
            name = self._read_line(
                "Enter your name (e.g., John Doe)", check=_check_nonempty
            )
    ...
    def _check_local_email(self) -> str:
        ...
        email = (
            scm.GIT.GetConfig(os.getcwd(), "user.email", scope="local") or ""
        )
        if email:
            self._println(
    ...
    def _check_use_sso(
        self, parts: urllib.parse.SplitResult, email: str
    ) -> bool:
    ...
    def _set_oauth_helper(
        ...
        self._set_config(
            f"credential.{_url_host_url(parts)}.useHttpPath",
            "yes",
            modify_all=True,
            scope=scope,
        )

    def _set_sso_rewrite(
        self, parts: urllib.parse.SplitResult, *, scope: scm.GitConfigScope
    ) -> None:
        self._set_url_rewrites(
            _url_gerrit_sso_url(parts),
            [_url_git_root_url(parts), _url_review_root_url(parts)],
            scope=scope,
        )

    def _clear_sso_rewrite(
        self, parts: urllib.parse.SplitResult, *, scope: scm.GitConfigScope
    ) -> None:
        self._set_url_rewrites(_url_gerrit_sso_url(parts), [], scope=scope)

    def _set_url_rewrite_override(
        self, parts: urllib.parse.SplitResult, *, scope: scm.GitConfigScope
    ) -> None:
    ...
    def _clear_url_rewrite_override(
        self, parts: urllib.parse.SplitResult, *, scope: scm.GitConfigScope
    ) -> None:
        self._set_url_rewrites(parts.geturl(), [], scope=scope)

    def _set_url_rewrites(
        self, new: str, old: Iterable[str], *, scope: scm.GitConfigScope
    ) -> None:
        ...
        """
        self._set_config(
            f"url.{new}.insteadOf", None, scope=scope, modify_all=True
        )
        for url in old:
            self._set_config(
                f"url.{new}.insteadOf", url, append=True, scope=scope
            )
...
def _url_git_root_url(parts: urllib.parse.SplitResult) -> str:
    ...
    return parts._replace(
        netloc=_url_git_host(parts), path="/", query="", fragment=""
    ).geturl()
...
def _url_review_root_url(parts: urllib.parse.SplitResult) -> str:
    ...
    """
    return parts._replace(
        netloc=_url_review_host(parts), path="/", query="", fragment=""
    ).geturl()
