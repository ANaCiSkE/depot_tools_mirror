    ...
    try:
        if inject_current:
            ...
            if upstream in git_common.tags():
                ...
                git_common.run(
                    "checkout", "--track", upstream, "-b", branch_name
                )
...
def main(args):
    if gclient_utils.IsEnvCog():
        print(
            "new-branch command is not supported in non-git environment.",
            file=sys.stderr,
        )
        return 1
    ...
    g.add_argument(
        "--upstream", metavar="REF", help="upstream branch (or tag) to track."
    )
    g.add_argument(
        "--inject-current",
        "--inject_current",
