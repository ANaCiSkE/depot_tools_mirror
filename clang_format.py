...
def FindClangFormatToolInChromiumTree():
    ...
    old_tool_path = os.path.join(
        bin_path, "clang-format" + gclient_paths.GetExeSuffix()
    )
    ...
    for path in possible_paths:
        if os.path.exists(path):
            return path
    raise NotFoundError(
        "File does not exist in either path: %s" % possible_paths
    )
...
def FindClangFormatScriptInChromiumTree(script_name):
    ...
    script_path = os.path.join(
        tools_path, "clang_format", "script", script_name
    )
