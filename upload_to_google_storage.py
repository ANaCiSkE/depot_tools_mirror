...
def _upload_worker(
    ...
    while True:
        ...
        if gsutil.check_call("ls", file_url)[0] == 0 and not force:
            ...
            if etag_match:
                stdout_queue.put(
                    "%d> File with url %s already exists"
                    % (thread_num, file_url)
                )
...
def upload_to_google_storage(
    ...
    for filename in input_filenames:
        ...
        if os.path.exists("%s.sha1" % filename) and skip_hashing:
            ...
            if not re.match("^([a-z0-9]{40})$", sha1_file.decode()):
                print(
                    "Invalid sha1 hash file %s.sha1" % filename, file=sys.stderr
                )
                return 1
    ...
    print(
        "Hashing %s files took %1f seconds"
...
def main():
    ...
    parser.add_option(
        ...
        "--gzip",
        metavar="ext",
        help="Gzip files which end in ext. ext is a comma-separated list",
    )
